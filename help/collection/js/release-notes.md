...
- Le utilità di base condivise sono ora pubblicate come pacchetto npm standalone ([@adobe/alloy-core](https://www.npmjs.com/package/@adobe/alloy-core)) per l&#39;utilizzo da parte di estensioni e integrazioni.
- Ora include il fuso orario IANA nel campo XDM `xdm.placeContext.ianaTimezone` quando `placeContext` è incluso nella variabile di configurazione [`context`](/help/collection/js/commands/configure/context.md).
- Brand Concierge: risolto un problema di ID sessione quando [`stickyConversationSession`](/help/collection/js/commands/configure/conversation.md) è disabilitato.
...
- [Le sostituzioni dello stream di dati](/help/datastreams/overrides.md) sono ora supportate all&#39;avvio di sessioni multimediali.
...
- È stato corretto un errore che si verificava durante il rendering degli elementi di contenuto predefiniti tramite [`applyPropositions`](commands/applypropositions.md).
- È stato corretto un errore CSS nelle azioni di spostamento e ridimensionamento di Adobe Target.
- La chiave `machineLearning` è stata rimossa dalle risposte [`sendEvent`](commands/sendevent/overview.md).
...
- Aggiunta del supporto per [Streaming Media Collection](commands/configure/streamingmedia.md).
...
- È stato aggiunto il supporto per [eventi di inizio e fine pagina](../use-cases/personalization/top-bottom-page-events.md).
...
- Aggiunta del supporto per [sostituzioni per comando dell&#39;ID dello stream di dati](/help/datastreams/overrides.md).
...
- È stato aggiunto il supporto per il callback di clic sul collegamento [`onBeforeLinkClickSend`](commands/configure/onbeforelinkclicksend.md).
...
- È stato risolto un problema a causa del quale [le proprietà dell&#39;hint client dell&#39;agente utente](../use-cases/client-hints.md) potevano essere impostate su un tipo errato.
...
- È stato aggiunto il supporto configurabile per [User-Agent Client Hints entropici elevati](../use-cases/client-hints.md).
- Aggiunta del supporto per il comando [`applyResponse`](commands/applyresponse.md). Consente la personalizzazione ibrida tramite [API Edge Network](https://developer.adobe.com/data-collection-apis/docs/api/).
...
- È ora possibile installare SDK come pacchetto [NPM](install/npm.md).
- È stato aggiunto il supporto per un&#39;opzione `out` durante la [configurazione del consenso predefinito](commands/configure/defaultconsent.md), che elimina tutti gli eventi fino alla ricezione del consenso (l&#39;opzione `pending` esistente accoda gli eventi e li invia dopo la ricezione del consenso).
- È ora possibile utilizzare il callback [`onBeforeEventSend`](commands/configure/onbeforeeventsend.md) per impedire l&#39;invio di un evento.
...
- Il comando [`getIdentity`](commands/getidentity.md) ora restituisce l&#39;ID dell&#39;area Edge insieme all&#39;identità.
- Gli avvisi e gli errori ricevuti dal server sono stati migliorati e vengono gestiti in modo più appropriato.
- È stato aggiunto il supporto per lo standard Consent 2.0 di Adobe per il comando [`setConsent`](commands/setconsent.md).
