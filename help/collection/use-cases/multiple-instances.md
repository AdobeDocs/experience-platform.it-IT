...
SDK consente di creare un’istanza separata per ogni proprietà aggiungendo un altro nome all’array nel codice di base. Nell&#39;esempio seguente vengono forniti due nomi, `titanium` e `copper`.
...
Di conseguenza, lo script crea due istanze di SDK. La funzione globale per interagire con la prima istanza è denominata `titanium` e la funzione globale per interagire con la seconda istanza è denominata `copper`.
