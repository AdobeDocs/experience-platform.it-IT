...
>[!IMPORTANT]
>
>Gli ID dispositivo di prime parti non sono compatibili con la funzionalità [cookie di terze parti](/help/tags/extensions/client/web-sdk/configure/identity.md) in Web SDK. Puoi utilizzare sia ID dispositivo di prima parte che cookie di terze parti, ma non entrambi simultaneamente.

## Prerequisiti {#prerequisites}

Prima di iniziare, assicurati di conoscere il funzionamento dei dati di identità nel Web SDK, inclusi gli ECID e `identityMap`. Per ulteriori informazioni, vedere la panoramica sui [dati di identità nel Web SDK](id-overview.md).
...
Si supponga di voler implementare Web SDK nel sito Web `example.com`. Il Web SDK invia i dati ad Edge Network al dominio `edge.adobedc.net`.

| Senza [!DNL CNAME] | Con [!DNL CNAME] |
|---------|----------|
| <ul><li>Il sito Web `example.com` utilizza il dominio Web SDK `edge.adobedc.net` per inviare dati ad Edge Network.</li><li>I cookie impostati da `edge.adobedc.net` sono considerati cookie di terze parti poiché non provengono dal dominio `example.com`. A seconda degli utenti e dei browser utilizzati, i cookie di terze parti potrebbero essere bloccati e i dati non raggiungono Edge Network.</li></ul> | <ul><li>Si crea un sottodominio in cui viene distribuito Web SDK, ad esempio `metrics.example.com`.</li><li>Hai impostato un record [!DNL CNAME] nel tuo sistema DNS in modo che `metrics.example.com` punti a `edge.adobedc.net`.</li><li>Quando il sito Web imposta i cookie tramite `metrics.example.com`, appariranno come provenienti da `example.com` (prime parti) invece di `edge.adobedc.net` (terze parti). In questo modo il cookie ID di prima parte ha meno probabilità di essere bloccato e viene quindi garantita una raccolta più accurata dei dati.</li></ul> |
...
Consulta la [documentazione sulla configurazione dello stream di dati](/help/datastreams/configure.md#advanced-options) per scoprire come impostare lo stream di dati.
