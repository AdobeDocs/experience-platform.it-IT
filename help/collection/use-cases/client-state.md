...
description: Scopri come Adobe Experience Platform Edge Network gestisce lo stato dei client
...
L’Edge Network stessa è senza stato (non mantiene la propria sessione). Tuttavia, in alcuni casi d’uso è necessaria la persistenza dello stato lato client, ad esempio:

* Identificazione coerente dei dispositivi
* Raccogliere e applicare il consenso degli utenti
* Mantenimento dell’ID della sessione di personalizzazione

Edge Network utilizza un protocollo di gestione dello stato, delegando l’aspetto di archiviazione al proprio client/SDK, e include voci di stato nelle risposte. Per i browser, le voci sono memorizzate come cookie.
...
Per supportare l&#39;assegnazione di più tag (ad esempio più istanze di SDK nella stessa proprietà, che potenzialmente fanno riferimento a organizzazioni diverse), tutte le voci di stato vengono automaticamente precedute da `kndctr_` e dall&#39;ID organizzazione sicuro per gli URL.

Quando il client SDK riceve un handle `state:store` nella risposta, deve eseguire le operazioni seguenti:
...
Quando si lavora con i client del browser, Edge Network può rendere le voci persistenti come cookie del browser. Questo consente il supporto dello stato di archiviazione trasparente, in quanto i browser rispettano il protocollo di gestione dello stato per impostazione predefinita.
...
Poiché le voci sono sempre associate a un ambito specifico (dispositivo/applicazione) in base alla loro definizione, solo il sottoinsieme compatibile con il contesto della richiesta corrente verrà scritto da Edge Network. Le voci non scritte sono
...
| `domain` | Stringa | Richiesto quando `cookiesEnabled: true`. Il dominio di primo livello in cui devono essere scritti i cookie. Edge Network utilizzerà questo valore per decidere se lo stato può essere mantenuto come cookie. |

Anche se il supporto dei cookie è abilitato tramite il flag `cookiesEnabled`, Adobe Experience Platform Edge Network scriverà le voci di stato solo se il dominio di primo livello della richiesta corrisponde a `domain` specificato dal chiamante. In caso di mancata corrispondenza, le voci vengono restituite in un handle `state:store`.
...
* Per i cookie di prime parti (`kndcrt_*`), il flag `Secure` viene impostato solo quando il contesto della richiesta è protetto (HTTPS) e quando il referente ([Intestazione HTTP referente](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Referer)) è anche HTTPS. Se il referente non è protetto (HTTP), il flag `Secure` viene omesso per consentire al Web SDK di leggerli. Impossibile leggere un cookie protetto da un contesto non sicuro.
