...
Adobe Experience Platform raccoglie e trasferisce i dati ad altri prodotti Adobe e a destinazioni di terze parti. Per inviare i dati dell’evento dall’applicazione all’Edge Network di Experience Platform, è importante comprendere queste tecnologie di base e configurarle in modo da fornire i dati alle destinazioni richieste, quando necessario.

Questa guida fornisce un tutorial di alto livello su come inviare un evento tramite Edge Network utilizzando le funzionalità di raccolta dati di Experience Platform. In particolare, il tutorial illustra i passaggi necessari per installare e configurare l’estensione tag Adobe Experience Platform Web SDK nell’interfaccia utente di Data Collection (precedentemente Adobe Experience Platform Launch).

>[!NOTE]
>
>Se non desideri utilizzare i tag, puoi anche scegliere di installare e configurare manualmente SDK, ma i passaggi successivi devono comunque essere completati come descritto di seguito.
>
...
Questa esercitazione utilizza l’interfaccia utente di Data Collection per creare uno schema, configurare uno stream di dati e installare il Web SDK. Per eseguire queste azioni nell&#39;interfaccia utente, è necessario disporre dell&#39;accesso ad almeno una proprietà Web insieme ai seguenti [diritti di proprietà](../tags/ui/administration/user-permissions.md#property-rights):
...
Per utilizzare i vari prodotti di raccolta dati menzionati in questa guida, devi anche avere accesso agli stream di dati e la possibilità di creare e gestire gli schemi. Se hai bisogno di accedere a una di queste funzioni, contatta il team del tuo account Adobe per ottenere l’accesso necessario. Se non hai acquistato Adobe Experience Platform, Adobe ti fornirà l’accesso necessario per utilizzare SDK senza alcun costo aggiuntivo.

Se disponi già dell&#39;accesso ad Experience Platform, assicurati di disporre di tutte le [autorizzazioni](../access-control/home.md#permissions) per le seguenti categorie abilitate:

* Modellazione dati
* Identità

Consulta la [panoramica dell&#39;interfaccia utente di controllo degli accessi](../access-control/ui/overview.md) per scoprire come concedere agli utenti le autorizzazioni per le funzionalità di Experience Platform.

## Riepilogo del processo

Il processo di configurazione della raccolta dati per il sito web può essere riassunto come segue:

1. [Crea uno schema](#schema) per determinare la struttura dei dati inviati ad Edge Network.
...
Una volta che sei in grado di inviare dati ad Edge Network, puoi anche [configurare l&#39;inoltro degli eventi](#event-forwarding) se la tua organizzazione dispone di una licenza per questo evento.

## Crea uno schema {#schema}

[Experience Data Model (XDM)](../xdm/home.md) è una specifica open-source che fornisce strutture e definizioni comuni per i dati sotto forma di schemi. In altre parole, XDM è un modo per strutturare e formattare i dati in modo utilizzabile da Edge Network e altre applicazioni Adobe Experience Cloud.
...
>[!NOTE]
>
>Gli schemi XDM sono molto personalizzabili. Invece di essere eccessivamente prescrittivi, i passaggi descritti di seguito si concentrano specificamente sui requisiti dello schema per Web SDK. Al di fuori di questi parametri, puoi definire la struttura rimanente dei dati come preferisci.
...
Viene visualizzata l’area di lavoro dello schema, con una struttura ad albero dello schema XDM che include i campi forniti dal gruppo di campi di Web SDK.
...
>[!NOTE]
>
>Se desideri utilizzare l&#39;[inoltro eventi](../tags/ui/event-forwarding/overview.md) (supponendo che la tua organizzazione disponga della licenza per questa funzionalità), devi abilitarlo per uno stream di dati nello stesso modo in cui abiliti i prodotti Adobe. I dettagli su questo processo sono trattati in una [sezione successiva](#event-forwarding).
...
## Installare e configurare il Web SDK {#install}

Dopo aver creato uno schema e un flusso di dati, il passaggio successivo consiste nell’installare e configurare Experience Platform Web SDK per iniziare a inviare dati ad Edge Network.
...
>* [Configura SDK](/help/web-sdk/commands/configure/overview.md)
>
>Inoltre, anche se desideri utilizzare solo l&#39;inoltro degli eventi, devi comunque installare e configurare SDK come descritto prima di configurare l&#39;inoltro degli eventi in un [passaggio successivo](#event-forwarding).

Il processo può essere riassunto come segue:

1. [Installare Adobe Experience Platform Web SDK in una proprietà tag](#install-sdk) per accedere alle relative funzionalità.
...
1. [Crea una regola](#rule) per indicare a SDK quando deve inviare dati ad Edge Network.
1. [Crea e installa una libreria](#library) per implementare la regola nel sito Web.

### Installare SDK su una proprietà tag {#install-sdk}
...
Viene visualizzata la pagina della panoramica della proprietà. Da qui, seleziona **[!UICONTROL Estensioni]** nell&#39;area di navigazione a sinistra, quindi seleziona **[!UICONTROL Catalogo]**. Individuare l&#39;elenco per Experience Platform Web SDK (se si desidera utilizzare la barra di ricerca per limitare i risultati) e selezionare **[!UICONTROL Installa]**.

![Installa Web SDK](./images/e2e/install-sdk.png)

Viene visualizzata la pagina di configurazione per SDK. La maggior parte dei valori richiesti viene compilata automaticamente con i valori predefiniti che puoi scegliere di modificare se lo desideri.

![Configurazione del Web SDK](./images/e2e/configure-sdk.png)

Prima di installare SDK, tuttavia, devi selezionare un flusso di dati in modo che sappia a dove inviare i dati. In **[!UICONTROL Datastreams]**, utilizza il menu a discesa per selezionare il datastream configurato in un [passaggio precedente](#datastream). Dopo aver impostato lo stream di dati, seleziona **[!UICONTROL Salva]** per completare l&#39;installazione di SDK nella proprietà.

![Imposta lo stream di dati e salva](./images/e2e/set-datastream.png)

### Creare un elemento dati XDM {#data-element}

Affinché SDK possa inviare dati ad Edge Network, questi devono essere mappati sullo schema XDM creato in un [passaggio precedente](#schema). Questa mappatura viene eseguita tramite l’utilizzo di un elemento dati.
...
Viene visualizzata la finestra di dialogo di configurazione per il tipo di oggetto XDM. La finestra di dialogo seleziona automaticamente la sandbox di Experience Platform e da qui puoi visualizzare tutti gli schemi creati in tale sandbox. Seleziona dall’elenco lo schema XDM creato in precedenza.
...
Dopo che hai salvato l’elemento dati, il passaggio successivo consiste nel creare una regola che lo invierà all’Edge Network ogni volta che si verifica un determinato evento sul sito web (ad esempio quando un cliente aggiunge un prodotto a un carrello).
...
>[!NOTE]
>
>Per ulteriori informazioni sui diversi tipi di eventi forniti dalle estensioni Web di Adobe, tra cui la relativa configurazione, consulta la [Guida di riferimento per le estensioni di Adobe](../tags/extensions/client/overview.md) nella documentazione dei tag.
...
Dopo aver configurato SDK per l’invio dei dati ad Edge Network, puoi impostare l’inoltro degli eventi per indicare all’Edge Network dove desideri che vengano consegnati i dati.
...
Questa guida fornisce una panoramica end-to-end di alto livello su come inviare dati ad Edge Network utilizzando Experience Platform Web SDK. Per ulteriori informazioni sui vari componenti e servizi coinvolti, consulta la documentazione accessibile dai collegamenti presenti in questa guida.
