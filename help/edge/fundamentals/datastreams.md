...
| Campo | Descrizione |
| --- | --- |
| [!UICONTROL Sandbox] | **(Obbligatorio)** Seleziona la sandbox Platform a cui desideri inviare i dati. Le sandbox sono partizioni virtuali in Adobe Experience Platform che ti consentono di isolare i dati e le implementazioni da altre parti della tua organizzazione.<br><br>Se crei un datastream senza selezionare una sandbox, puoi comunque selezionare una sandbox in un secondo momento.<br><br>Una volta creato un datastream e selezionato un sandbox, questo non può essere modificato. La [!UICONTROL Sandbox] il campo di selezione non è pertanto disponibile quando si modifica un datastream esistente con una sandbox selezionata.<br><br> La [!UICONTROL Sandbox] il campo di selezione non è pertanto disponibile quando si modifica un datastream esistente.<br><br>Per ulteriori dettagli sul ruolo delle sandbox in Experience Platform, consulta la sezione [documentazione sandbox](../../sandboxes/home.md). |
