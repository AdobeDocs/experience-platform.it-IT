...
L’SDK per web è attualmente disponibile al pubblico e può essere utilizzato per inviare dati ai prodotti Adobe Experience Cloud. La capacità di inviare dati a soluzioni di terze parti sarà presto disponibile. L’SDK è gratuito, è ospitato da Adobe gratuitamente e può essere scaricato in modo da poterlo ospitare sui tuoi server, se necessario, gratuitamente. Platform Web SDK richiede l’accesso alle configurazioni dello stream di dati e al generatore di schemi XDM di Adobe Experience Platform per consentire ai server di Adobe di gestire correttamente i dati in entrata provenienti dall’SDK. Se desideri ottenere l’accesso, contatta il tuo Customer Success Manager (CSM) per avviare il processo di richiesta.
