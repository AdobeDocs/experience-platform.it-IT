...
I dati verranno scritti in [!DNL Experience Platform] utilizzando [API di acquisizione in batch](https://developer.adobe.com/experience-platform-apis/references/batch-ingestion/).  La scrittura dei dati è un processo asincrono. Quando i dati vengono scritti in Adobe Experience Platform, viene creato un batch contrassegnato come completato solo dopo che i dati sono stati scritti completamente.
...
Una volta elaborati i dati, lo strumento ETL li riscriverà in [!DNL Experience Platform] utilizzando [API di acquisizione in batch](https://developer.adobe.com/experience-platform-apis/references/batch-ingestion/). Prima di poter aggiungere dati a un set di dati, questi devono essere collegati a un batch che verrà successivamente caricato in un set di dati specifico.
