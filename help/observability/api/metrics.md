...
Puoi recuperare i dati delle metriche effettuando una richiesta di GET all&#39;endpoint `/metrics`, specificando le metriche tramite l&#39;uso di parametri di query.

**Formato API**

Nel parametro `metric` deve essere specificata almeno una metrica. Altri parametri di query sono facoltativi per filtrare i risultati.
...
| --- | --- |
| `{METRIC}` | La metrica da esporre. Quando si combinano più metriche in una singola chiamata, è necessario utilizzare una e commerciale (`&`) per separare le singole metriche. Ad esempio, `metric={METRIC_1}&metric={METRIC_2}`. |
| `{ID}` | Identificatore per una particolare risorsa [!DNL Platform] di cui si desidera esporre le metriche. Questo ID può essere facoltativo, obbligatorio o non applicabile a seconda delle metriche utilizzate. Consulta l&#39; [appendice](#available-metrics) per un elenco delle metriche disponibili, inclusi gli ID supportati (obbligatori e facoltativi) per ciascuna metrica. |
...
Una risposta corretta restituisce un elenco di oggetti, ciascuno contenente una marca temporale all&#39;interno della `dateRange` fornita e i valori corrispondenti per le metriche specificate nel percorso della richiesta. Se nel percorso di richiesta è incluso il `id` di una risorsa [!DNL Platform], i risultati verranno applicati solo a tale risorsa. Se `id` viene omesso, i risultati verranno applicati a tutte le risorse applicabili all&#39;interno dell&#39;organizzazione IMS.
...
Puoi recuperare i dati delle metriche effettuando una richiesta di POST all&#39;endpoint `/metrics`, specificando le metriche che desideri recuperare nel payload.
...
| `name` | Il nome di una metrica riconosciuta da Observability Insights. Vedere l&#39; [appendice](#available-metrics) per un elenco completo dei nomi delle metriche accettate. |
| `filters` | Campo facoltativo che consente di filtrare le metriche in base a specifici set di dati. Il campo è un array di oggetti (uno per ciascun filtro), con ogni oggetto contenente le seguenti proprietà: <ul><li>`name`: Il tipo di entità con cui filtrare le metriche. Attualmente, è supportato solo `dataSets`.</li><li>`value`: ID di uno o più set di dati. Più ID di set di dati possono essere forniti come una singola stringa, con ciascun ID separato da caratteri a barre verticali (`|`).</li><li>`groupBy`: Se impostato su true, indica che il corrispondente  `value` rappresenta più set di dati i cui risultati della metrica devono essere restituiti separatamente. Se impostato su false, i risultati delle metriche per tali set di dati vengono raggruppati.</li></ul> |
| `aggregator` | Specifica la funzione di aggregazione da utilizzare per raggruppare più record serie temporali in singoli risultati. Per informazioni dettagliate sugli aggregatori disponibili, fare riferimento alla [documentazione OpenTSDB](http://opentsdb.net/docs/build/html/user_guide/query/aggregators.html). |
| `downsample` | Campo facoltativo che consente di specificare una funzione di aggregazione per ridurre la frequenza di campionamento dei dati delle metriche ordinando i campi in intervalli (o &quot;secchi&quot;). L&#39;intervallo per il downsampling è determinato dalla proprietà `granularity`. Per informazioni dettagliate sul downsampling, fare riferimento alla [documentazione OpenTSDB](http://opentsdb.net/docs/build/html/user_guide/query/downsampling.html). |
...
Le tabelle seguenti elencano tutte le metriche esposte dal servizio [!DNL Observability Insights], suddivise per [!DNL Platform]. Ogni metrica include una descrizione e il parametro di query ID accettato.
...
La tabella seguente delinea le metriche per Adobe Experience Platform [!DNL Data Ingestion]. Le metriche in **bold** sono metriche di assimilazione in streaming.
...
| Metrica Insights | Descrizione | Parametro query ID |
| ---- | ---- | ---- |
| timeseries.identity.dataset.recordsuccess.count | Numero di record scritti nell&#39;origine dati da [!DNL Identity Service] per un set di dati o tutti i set di dati. | ID set di dati |
...
| timeseries.identity.graph.imsorg.graphstrength.uniqueidentities.count | Numero di identità univoche memorizzate nel grafico dell&#39;identità per l&#39;organizzazione IMS per una particolare intensità del grafico (&quot;sconosciuto&quot;, &quot;debole&quot; o &quot;forte&quot;). | Forza del grafico (**Obbligatorio**) |
...
| Metrica Insights | Descrizione | Parametro query ID |
| ---- | ---- | ---- |
| timeseries.profiles.dataset.recordread.count | Numero di record letti da [!DNL Data Lake] di [!DNL Profile], per un set di dati o per tutti i set di dati. | ID set di dati |
...
| timeseries.profiles.dataset.batchsuccess.count | Numero di batch [!DNL Profile] assimilati per un set di dati o per tutti i set di dati. | ID set di dati |
...
| platform.ups.ingest.streaming.request.m1_rate | Tasso richieste in arrivo. | Organizzazione IMS (**Richiesto**) |
| platform.ups.ingest.streaming.access.put.success.m1_rate | Tasso di successo dell&#39;ingestione. | Organizzazione IMS (**Richiesto**) |
...
| platform.ups.ingest.streaming.request.error.created.outOfOrder.m1_rate | Frequenza dei record con marca temporale fuori ordine per la creazione di richieste per un dataset. | ID set di dati (**Richiesto**) |
| platform.ups.profile-commons.ingest.streaming.dataSet.record.created.timestamp | Timestamp dell&#39;ultima richiesta di creazione record per un dataset. | ID set di dati (**Richiesto**) |
| platform.ups.ingest.streaming.request.error.updated.outOfOrder.m1_rate | Frequenza dei record con marca temporale fuori ordine per la richiesta di aggiornamento per un dataset. | ID set di dati (**Richiesto**) |
| platform.ups.profile-commons.ingest.streaming.dataSet.record.updated.timestamp | Timestamp per l&#39;ultima richiesta di aggiornamento record per un dataset. | ID set di dati (**Richiesto**) |
| platform.ups.ingest.streaming.record.size.m1_rate | Dimensione media del record. | Organizzazione IMS (**Richiesto**) |
| platform.ups.ingest.streaming.records.updated.m15_rate | Frequenza delle richieste di aggiornamento per i record acquisiti per un set di dati. | ID set di dati (**Richiesto**) |

### Messaggi di errore

Le risposte dall&#39;endpoint `/metrics` possono restituire messaggi di errore a determinate condizioni. Questi messaggi di errore vengono restituiti nel formato seguente:
...
| --- | --- | --- |
| `INSGHT-1000-400` | Payload di richiesta non valido | Si è verificato un errore nel payload della richiesta. Assicurarsi che la formattazione del payload corrisponda esattamente a quella mostrata in [sopra](#v2). Uno dei possibili motivi può attivare questo errore:<ul><li>Campi obbligatori mancanti, ad esempio `aggregator`</li><li>Metriche non valide</li><li>La richiesta contiene un aggregatore non valido</li><li>Una data di inizio ha luogo dopo una data di fine</li></ul> |
...
| `INSGHT-1003-401` | Errore di convalida sandbox | Impossibile elaborare la richiesta a causa di un errore di convalida sandbox. Prima di riprovare, verificate che il nome della sandbox fornito nell&#39;intestazione `x-sandbox-name` rappresenti una sandbox valida e abilitata per l&#39;organizzazione IMS. |
