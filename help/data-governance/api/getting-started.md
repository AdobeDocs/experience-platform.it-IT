...
`core` le risorse sono quelle definite e gestite dall&#39;Adobe, mentre `custom` le risorse sono quelle create e gestite dalla tua organizzazione e sono quindi uniche e visibili solo per la tua organizzazione. Di conseguenza, operazioni di inserimento nell’elenco e ricerca (`GET`) sono le uniche operazioni consentite su `core` risorse, mentre operazioni di elenco, ricerca e aggiornamento (`POST`, `PUT`, `PATCH`e `DELETE`) sono disponibili per `custom` risorse.
