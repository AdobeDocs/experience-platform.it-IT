...
`core` le risorse sono quelle definite e mantenute dall&#39;Adobe, mentre `custom` le risorse sono quelle create e gestite dalla tua organizzazione e sono quindi univoche e visibili solo alla tua organizzazione IMS. Di conseguenza, le operazioni di elenco e ricerca (`GET`) sono le uniche operazioni consentite il `core` risorse, mentre le operazioni di elenco, ricerca e aggiornamento (`POST`, `PUT`, `PATCH`, e `DELETE`) sono disponibili per `custom` risorse.
