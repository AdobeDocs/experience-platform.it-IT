...
La creazione di un server di destinazione è il primo passaggio nella creazione di una destinazione personalizzata con Destination SDK. Il server di destinazione include le opzioni di configurazione per [server](../../functionality/destination-server/server-specs.md) e [templare](../../functionality/destination-server/templating-specs.md) specifiche, [formato messaggio](../../functionality/destination-server/message-format.md)e [formattazione dei file](../../functionality/destination-server/file-formatting.md) (per le destinazioni basate su file).

Questa pagina esemplifica la richiesta API e il payload che puoi utilizzare per creare il server di destinazione utilizzando `/authoring/destination-servers` Endpoint API.
...
Puoi creare una nuova configurazione del server di destinazione creando un `POST` richiesta al `/authoring/destination-servers` punto finale.
...
A seconda del tipo di destinazione creato, è necessario configurare un tipo di server di destinazione leggermente diverso. Vedi nelle schede seguenti gli esempi di server di destinazione per ogni tipo di destinazione supportato nella Destination SDK.

I payload di esempio riportati di seguito includono tutti i parametri supportati da ciascun tipo di server di destinazione. Non è necessario includere tutti i parametri nella richiesta. Il payload è personalizzabile in base alle tue esigenze.

Seleziona ogni scheda qui sotto per visualizzare le richieste API corrispondenti.
...
| `urlBasedDestination.url.templatingStrategy` | Stringa | *Obbligatorio.* <ul><li>Utilizzo `PEBBLE_V1` se Adobe deve trasformare l’URL nel `value` campo sottostante. Usa questa opzione se hai un endpoint come `https://api.moviestar.com/data/{{customerData.region}}/items`, dove `region` parte può differire tra i clienti. In questo caso è inoltre necessario configurare `region` come [campo dati cliente](../../functionality/destination-configuration/customer-data-fields.md) in [configurazione di destinazione](../destination-configuration/create-destination-configuration.md) </li><li> Utilizzo `NONE` se non è necessaria alcuna trasformazione sul lato Adobe, ad esempio se si dispone di un endpoint come: `https://api.moviestar.com/data/items`.</li></ul> |
...
È necessario creare un [!DNL Amazon S3] server di destinazione simile a quello mostrato di seguito quando si configura un server basato su file [!DNL Amazon S3] destinazione.
...
**Crea un [!DNL SFTP] server di destinazione**

È necessario creare un [!DNL SFTP] server di destinazione simile a quello mostrato di seguito quando si configura un server basato su file [!DNL SFTP] destinazione.
...
**Crea un [!DNL Azure Data Lake Storage] server di destinazione**

È necessario creare un [!DNL Azure Data Lake Storage] server di destinazione simile a quello mostrato di seguito quando si configura un server basato su file [!DNL Azure Data Lake Storage] destinazione.
...
**Crea un [!DNL Azure Blob Storage] server di destinazione**

È necessario creare un [!DNL Azure Blob Storage] server di destinazione simile a quello mostrato di seguito quando si configura un server basato su file [!DNL Azure Blob Storage] destinazione.
...
**Crea un [!DNL Data Landing Zone (DLZ)] server di destinazione**

È necessario creare un [!DNL Data Landing Zone (DLZ)] server di destinazione simile a quello mostrato di seguito quando si configura un server basato su file [!DNL Data Landing Zone (DLZ)] destinazione.
...
**Crea un [!DNL Google Cloud Storage] server di destinazione**

È necessario creare un [!DNL Google Cloud Storage] server di destinazione simile a quello mostrato di seguito quando si configura un server basato su file [!DNL Google Cloud Storage] destinazione.
...
>[!TAB Server di schema dinamico]

**Creare un server di schema dinamico**

È necessario creare un server di schema dinamico simile a quello mostrato di seguito quando si configura una destinazione che recupera il proprio schema di profilo dall&#39;endpoint API. Invece di uno schema statico, uno schema dinamico non utilizza un `profileFields` array. Al contrario, gli schemi dinamici utilizzano un server di schema dinamico che si connette all’API da cui recupera la configurazione dello schema.
...
| `destinationServerType` | Stringa | *Obbligatorio.* Imposta su `URL_BASED` per i server schema dinamici. |
...
| `urlBasedDestination.url.value` | Stringa | *Obbligatorio.* Compila l’indirizzo dell’endpoint API a cui Experience Platform deve connettersi e recuperare i campi dello schema da compilare come campi di destinazione nel passaggio di mappatura del flusso di lavoro di attivazione. |
| `httpTemplate.httpMethod` | Stringa | *Obbligatorio.* Il metodo che Adobe utilizzerà nelle chiamate al server. Per i server schema dinamici, utilizza `GET`. |
| `responseFields.templatingStrategy` | Stringa | *Obbligatorio.* Seleziona `PEBBLE_V1`. |
| `responseFields.value` | Stringa | *Obbligatorio.* Questa stringa è il modello di trasformazione con sequenza di caratteri che trasforma la risposta ricevuta dall’API partner nello schema partner che verrà visualizzata nell’interfaccia utente di Platform. <br> <ul><li> Per informazioni su come scrivere il modello, leggi il [Utilizzo della sezione di template](../../functionality/destination-server/message-format.md#using-templating). </li><li> Per ulteriori informazioni sull&#39;escape dei caratteri, consulta la [RFC JSON standard, sezione sette](https://tools.ietf.org/html/rfc8259#section-7). </li><li> Per un esempio di trasformazione semplice, consulta la sezione [Attributi del profilo](../../functionality/destination-server/message-format.md#attributes) trasformazione. </li></ul> |
...
* [Utilizza Destination SDK per configurare una destinazione di streaming](../../guides/configure-destination-instructions.md#create-server-template-configuration)
