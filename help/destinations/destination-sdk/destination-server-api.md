...
## Creare una configurazione per un server di destinazione streaming {#create}

Puoi creare una nuova configurazione del server di destinazione per una destinazione di streaming effettuando una richiesta di POST al `/authoring/destination-servers` punto finale.
...
Puoi recuperare un elenco di tutte le configurazioni del server di destinazione per la tua organizzazione effettuando una richiesta di GET al `/authoring/destination-servers` punto finale.
...
La seguente richiesta recupererà l’elenco delle configurazioni del server di destinazione a cui hai accesso, in base alla configurazione dell’organizzazione e della sandbox.
...
La risposta seguente restituisce lo stato HTTP 200 con un elenco di configurazioni del server di destinazione a cui hai accesso, in base all&#39;ID organizzazione e al nome della sandbox utilizzati. Uno `instanceId` corrisponde al modello per un server di destinazione. La risposta viene troncata per brevità.
