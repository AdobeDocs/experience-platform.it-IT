...
Puoi recuperare un elenco di tutte le destinazioni inviate per la pubblicazione per la tua organizzazione effettuando una richiesta di GET al `/authoring/destinations/publish` punto finale.
...
La seguente richiesta recupera l’elenco delle destinazioni inviate per la pubblicazione a cui hai accesso, in base alla configurazione dell’organizzazione e della sandbox.
...
La risposta seguente restituisce lo stato HTTP 200 con un elenco di destinazioni inviate per la pubblicazione a cui hai accesso, in base all’ID organizzazione e al nome della sandbox utilizzati. Uno `configId` corrisponde alla richiesta di pubblicazione per una destinazione.
