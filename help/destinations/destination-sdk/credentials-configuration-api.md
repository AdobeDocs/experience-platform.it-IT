...
Puoi recuperare un elenco di tutte le configurazioni di credenziali per la tua organizzazione effettuando una richiesta di GET al `/authoring/credentials` punto finale.
...
La seguente richiesta recupererà l’elenco delle configurazioni di credenziali a cui hai accesso, in base alla configurazione dell’organizzazione e della sandbox.
...
La risposta seguente restituisce lo stato HTTP 200 con un elenco di configurazioni di credenziali a cui hai accesso, in base all&#39;ID organizzazione e al nome della sandbox utilizzati. Uno `instanceId` corrisponde al modello per una configurazione di credenziali. La risposta viene troncata per brevità.
