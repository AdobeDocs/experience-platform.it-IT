...
Per modificare più pianificazioni contemporaneamente, selezionare i tipi di pubblico utilizzando le caselle di controllo sul lato sinistro della schermata, quindi selezionare **[!UICONTROL Edit schedule]**. La pianificazione configurata verrà quindi applicata a tutti i file esportati per i tipi di pubblico selezionati.
...
1. Utilizza il selettore **[!UICONTROL Frequency]** per selezionare la frequenza di esportazione:
   ...
   * **[!UICONTROL Monthly]**: selezionare la data di inizio e le esportazioni successive avverranno in quella data del mese fino alla data di fine selezionata. Per i mesi con meno di 30 o 31 giorni, l’esportazione viene eseguita l’ultimo giorno del mese.
   ...
   > * [Archiviazione BLOB Azure](../catalog/cloud-storage/azure-blob.md)
...
Utilizzare l&#39;opzione **[!UICONTROL Scheduled]** per fare in modo che il processo di attivazione venga eseguito a un orario fisso. Questa opzione assicura che i dati del profilo di Experience Platform vengano esportati ogni giorno alla stessa ora. Tuttavia, i profili esportati potrebbero non essere quelli più aggiornati, a seconda che il processo di segmentazione batch sia stato completato prima dell’avvio del processo di attivazione.
   ...
   Durante la configurazione delle pianificazioni di esportazione, impostare l&#39;ora di inizio almeno **1 ora** dopo il completamento del flusso di attivazione. La propagazione delle attivazioni del pubblico attraverso il sistema può richiedere fino a 1 ora. Se pianifichi un’esportazione che verrà eseguita prima di 1 ora dopo l’attivazione, l’esportazione pianificata potrebbe non essere eseguita.
...
1. Il campo selezionato per l&#39;esportazione viene ora visualizzato nella vista di mappatura. Se lo desideri, puoi modificare il nome dell’intestazione nel file esportato. A questo scopo, seleziona l’icona nel campo di destinazione.
   ...
   >[!NOTE]
   >
   >I punti (`.`) non sono supportati nei nomi dei campi nei file esportati. Se un nome di campo include punti (ad esempio `person.name.firstName`), ogni punto verrà sostituito da un carattere di sottolineatura (`_`) nel nome della colonna esportata. `person.name.firstName`, ad esempio, diventerà `person_name_firstName` nel file esportato.
...
>title="Output gerarchico abilitato"
>abstract="Questa destinazione supporta l&#39;output gerarchico perché l&#39;opzione Abilita esportazione di array, mappe e oggetti è attivata. È possibile esportare array di primo livello, elementi di array o più campi dallo stesso array in un&#39;unica mappatura. Per informazioni, consulta la documentazione."

>[!CONTEXTUALHELP]
>id="platform_destinations_enrichment_attributes_source_field"
>title="Campo di origine"
>abstract="Selezionate un attributo di arricchimento da esportare. Per i campi all’interno di un array, l’origine viene compilata automaticamente con un’espressione di trasformazione. Per esportare più campi in una mappatura, aggiungi prima un campo, quindi modifica l’espressione di origine. Per informazioni, consulta la documentazione."

>[!CONTEXTUALHELP]
>id="platform_destinations_enrichment_attributes_target_field"
>title="Campo di destinazione"
>abstract="Il campo di destinazione viene compilato automaticamente con il nome del campo di origine. Modificalo per utilizzare un alias diverso se desideri che il campo abbia un nome diverso nei file esportati."
