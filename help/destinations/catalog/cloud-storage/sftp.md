...
Crea una connessione in uscita al server SFTP per esportare periodicamente file di dati delimitati da [!DNL Adobe Experience Platform].
...
* Per connetterti al percorso di archiviazione SFTP utilizzando l&#39;interfaccia utente di Experience Platform, leggi le sezioni [Connetti alla destinazione](#connect) e [Attiva i tipi di pubblico a questa destinazione](#activate) di seguito.
...
| [Esportazioni set di dati](/help/catalog/datasets/overview.md) | Sì | Raccolte di dati strutturati archiviati nel Data Lake [!DNL Adobe Experience Platform]. | Reporting, flussi di lavoro di data science |
...
## Requisiti di connessione al server SFTP {#sftp-connection-requirements}

Per garantire un’esportazione dei dati corretta, devi configurare il server SFTP di destinazione in modo da consentire un numero sufficiente di connessioni simultanee. Se il server SFTP limita il numero di connessioni simultanee, si potrebbero verificare errori nei processi di esportazione, in particolare quando si esportano più tipi di pubblico o set di dati contemporaneamente.

**Consiglio**
Per prestazioni ottimali, il server SFTP deve consentire almeno una connessione simultanea per ogni pubblico o set di dati da esportare. Come minimo, il server deve supportare almeno il 30% del numero totale di tipi di pubblico o set di dati pianificati per l’esportazione contemporaneamente.

**Esempio**\
Se pianifichi esportazioni simultanee per 100 tipi di pubblico o set di dati, il server SFTP deve consentire almeno 30 connessioni simultanee.

La configurazione corretta dei limiti di connessione del server SFTP consente di evitare esportazioni non riuscite e garantisce la consegna affidabile dei dati da [!DNL Adobe Experience Platform].
...
Se selezioni il tipo di autenticazione **[!UICONTROL SFTP with password]** per la connessione alla posizione SFTP:
...
* **[!UICONTROL Domain]**: indirizzo del percorso di archiviazione SFTP;
...
Se selezioni il tipo di autenticazione **[!UICONTROL SFTP with SSH key]** per la connessione alla posizione SFTP:

![Autenticazione della chiave SSH di destinazione SFTP.](../../assets/catalog/cloud-storage/sftp/sftp-ssh-key-authentication.png)

* **[!UICONTROL Domain]**: inserisci l&#39;indirizzo IP o il nome di dominio dell&#39;account SFTP
* **[!UICONTROL Port]**: la porta utilizzata dal percorso di archiviazione SFTP;
* **[!UICONTROL Username]**: nome utente per accedere al percorso di archiviazione SFTP;
* **[!UICONTROL SSH Key]**: la chiave SSH privata utilizzata per accedere al percorso di archiviazione SFTP. La chiave privata deve essere una stringa in formato RSA con codifica Base64 e non deve essere protetta da password.
* **[!UICONTROL Encryption key]**: facoltativamente, è possibile allegare la chiave pubblica in formato RSA per aggiungere la crittografia ai file esportati. Visualizza un esempio di chiave di crittografia formattata correttamente nell’immagine seguente.
...
* **[!UICONTROL Name]**: immettere un nome che consenta di identificare questa destinazione nell&#39;interfaccia utente di Experience Platform;
* **[!UICONTROL Description]**: immettere una descrizione per questa destinazione;
* **[!UICONTROL Folder path]**: immetti il percorso della cartella nel percorso SFTP in cui verranno esportati i file.
