...
Entro **[!UICONTROL Destinations]** > **[!UICONTROL Catalog]**, cerca [!DNL Outreach]. In alternativa, è possibile individuarlo nella categoria CRM.
...
* **[!UICONTROL Username]**: indirizzo e-mail dell&#39;account [!DNL Outreach].
...
1. Nella finestra [!UICONTROL Select target field], selezionare il tipo di campo di destinazione a cui si desidera mappare il campo di origine.
   ...
   * **[!UICONTROL Select custom attributes]**: selezionare questa opzione per associare il campo di origine a un attributo personalizzato definito nel campo [!UICONTROL Attribute name]. Per un elenco completo degli attributi supportati, consulta la [[!DNL Outreach] documentazione del prospect](https://api.outreach.io/api/v2/docs#prospect).
...
* A questo scopo, seleziona ogni segmento, quindi immetti il valore numerico corrispondente che corrisponde al campo *Campo personalizzato `N` Etichetta* di [!DNL Outreach] nel campo **[!UICONTROL Mapping ID]**.
...
Per correggere questo errore, verifica che [!UICONTROL Mapping ID] fornito in Experience Platform per il pubblico [!DNL Outreach] sia valido ed esista in [!DNL Outreach].
