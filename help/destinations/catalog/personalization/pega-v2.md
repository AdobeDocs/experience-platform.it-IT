...
Utilizzare la destinazione (V2) [!DNL Pega CDH Realtime Audience] in [!DNL Adobe Experience Platform] per inviare gli attributi del profilo e i dati sull&#39;appartenenza al pubblico a [!DNL Pega Customer Decision Hub] per le decisioni sulle azioni migliori successive.

L&#39;appartenenza al pubblico di profilo da [!DNL Adobe Experience Platform], quando caricata in [!DNL Pega Customer Decision Hub], può essere utilizzata come predittore nei modelli adattivi e contribuire a fornire i dati contestuali e comportamentali corretti per le decisioni da intraprendere al meglio.
...
Per aiutarti a capire meglio come e quando utilizzare la destinazione [!DNL Customer Decision Hub], ecco alcuni esempi di casi d&#39;uso che i clienti [!DNL Adobe Experience Platform] possono risolvere utilizzando questa destinazione.
...
|---|---|---|
| `CustomerID` | ID cliente | Identificatore utente comune che identifica in modo univoco un profilo in [!DNL Pega Customer Decision Hub] e [!DNL Adobe Experience Platform]. |
...
Nel passaggio [!UICONTROL Mapping], seleziona un identificatore univoco dallo schema di unione e dagli altri campi XDM da esportare nella destinazione.
...
* Selezionare un&#39;identità di origine che identifica in modo univoco un profilo in [!DNL Adobe Experience Platform] e [!DNL Pega Customer Decision Hub]. Ad esempio: `CustomerID`.
