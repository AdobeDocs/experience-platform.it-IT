...
description: Scopri come interagire con più proprietà di Experienci Platform Web SDK.
...
Di conseguenza, lo script crea due istanze dell’SDK. La funzione globale per l’interazione con la prima istanza è denominata `titanium` e la funzione globale per interagire con la seconda istanza è denominata `copper`.

Creando due istanze separate, ciascuna può essere configurata per una proprietà diversa. Qualsiasi comunicazione o persistenza dei dati che si verifica a causa dell’interazione con `titanium` è tenuto isolato da `copper`.
...
>[!IMPORTANT]
>
>Per evitare conflitti con i cookie, ogni istanza dell’SDK web deve avere il proprio univoco `edgeConfigId` e il suo unico `orgId`.
