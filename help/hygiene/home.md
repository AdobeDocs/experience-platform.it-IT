...
>[!IMPORTANT]
>
>L’igiene dei dati è attualmente disponibile solo per le organizzazioni che hanno acquistato **Schermo sanitario Adobe** o **Adobe Privacy &amp; Security Shield**.
...
Queste attività possono essere eseguite utilizzando [[!UICONTROL Igiene dei dati] Area di lavoro interfaccia utente](#ui) o [API di igiene dei dati](#api). Quando viene eseguito un processo di igiene dei dati, il sistema fornisce aggiornamenti di trasparenza in ogni fase del processo. Consulta la sezione su [tempistiche e trasparenza](#timelines-and-transparency) per ulteriori informazioni su come ogni tipo di processo viene rappresentato nel sistema.

## [!UICONTROL Igiene dei dati] Area di lavoro interfaccia utente {#ui}

Il [!UICONTROL Igiene dei dati] Workspace nell’interfaccia utente di Platform consente di configurare e pianificare le operazioni di igiene dei dati, garantendo che i record vengano mantenuti come previsto.

Per i passaggi dettagliati sulla gestione delle attività di igiene dei dati nell’interfaccia utente, consulta [Guida dell’interfaccia utente di Data Hygiene](./ui/overview.md).

## API di igiene dei dati {#api}

Il [!UICONTROL Igiene dei dati] L’interfaccia utente è basata sull’API di igiene dei dati, i cui endpoint sono disponibili per l’uso diretto se preferisci automatizzare le attività di igiene dei dati. Consulta la [Guida dell’API di igiene dei dati](./api/overview.md) per ulteriori informazioni.

## Timeline e trasparenza

Le richieste di eliminazione dei record e di scadenza dei set di dati hanno ciascuno le proprie timeline di elaborazione e forniscono aggiornamenti di trasparenza in punti chiave nei rispettivi flussi di lavoro.
...
| --- | --- | --- |
| Richiesta inviata | 0 ore | Un amministratore di dati o un analista della privacy invia una richiesta affinché un set di dati scada in un determinato momento. La richiesta è visibile in [!UICONTROL Interfaccia utente di igiene dei dati] dopo l’invio, rimane in sospeso fino alla scadenza pianificata, dopo la quale la richiesta verrà eseguita. |
...
| Eliminazione definitiva completata | 14 giorni | Tutti i dati relativi al set di dati vengono eliminati dal data lake. Il [stato del processo di igiene](./ui/browse.md#view-details) che ha eliminato il set di dati viene aggiornato di conseguenza. |
