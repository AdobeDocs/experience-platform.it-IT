...
# Guida dell’interfaccia utente per la preparazione dei dati {#data-prep-ui-guide}
...
| --- | --- |
| [!UICONTROL Mapped fields] | Visualizza il numero totale di campi sorgente mappati a un campo XDM di destinazione, indipendentemente dagli errori. |
| [!UICONTROL Required fields] | Visualizza il numero di campi di mappatura obbligatori. |
| [!UICONTROL Identity fields] | Visualizza il numero totale di campi di mappatura definiti come identità. Questi campi di mappatura sono rappresentati da un&#39;icona di impronta digitale. |
| [!UICONTROL Errors] | Visualizza il numero di campi di mappatura errati. |
...
| Opzione | Descrizione |
| --- | --- |
| [!UICONTROL Search source fields] | Utilizza la barra di ricerca per passare a un campo di origine specifico. |
| [!UICONTROL All fields] | Seleziona **[!UICONTROL All fields]** per visualizzare un menu a discesa di opzioni in base alle quali filtrare le mappature. Le opzioni di filtro disponibili includono:<ul><li>**[!UICONTROL Required fields]**: filtra l&#39;interfaccia per visualizzare solo i campi necessari per completare il flusso di lavoro.</li><li> **[!UICONTROL Identity fields]**: filtra l&#39;interfaccia in modo da visualizzare solo i campi contrassegnati come identità.</li><li>**[!UICONTROL Mapped fields]**: filtra l&#39;interfaccia per visualizzare solo i campi già mappati.</li><li>**[!UICONTROL Unmapped fields]**: filtra l&#39;interfaccia in modo da visualizzare solo i campi non ancora mappati.</li><li>**[!UICONTROL Fields with errors]**: filtra l&#39;interfaccia in modo da visualizzare solo i campi con errori.</li></ul> |
| [!UICONTROL New field type] | Selezionare **[!UICONTROL New field type]** per aggiungere un nuovo campo o un campo calcolato. Per ulteriori informazioni, leggere la sezione relativa all&#39;[aggiunta di un nuovo tipo di campo](#add-a-new-field-type). |
| [!UICONTROL Import mappings] | Selezionare **[!UICONTROL Import mappings]** per importare i mapping da un file o un flusso di dati esistente. Per ulteriori informazioni, leggere la sezione su [importazione di mapping](#import-mapping). |
| [!UICONTROL Validate] | Selezionare **[!UICONTROL Validate]** per verificare la presenza di errori nei mapping. |
| [!UICONTROL Download template] | Seleziona **[!UICONTROL Download template]** per esportare e scaricare un file CSV delle tue mappature. |
| [!UICONTROL Preview data] | Selezionare **[!UICONTROL Preview data]** per utilizzare il pannello di anteprima e controllare la struttura e il contenuto del set di dati di origine. |
| [!UICONTROL Clear all] | Selezionare **[!UICONTROL Clear all]** per eliminare tutti i mapping nell&#39;interfaccia. |

{style="table-layout:auto"}

### Aggiungi un nuovo tipo di campo {#add-a-new-field-type}

È possibile aggiungere un nuovo campo di mappatura o un campo calcolato selezionando **[!UICONTROL New field type]**.

#### Nuovo campo di mappatura

Per aggiungere un nuovo campo di mappatura, selezionare **[!UICONTROL New field type]**, quindi selezionare **[!UICONTROL Add new field]** dal menu a discesa visualizzato.

![Interfaccia di mappatura con il pulsante &quot;Aggiungi nuovo campo&quot; selezionato.](../images/ui/mapping/add_new_field.png)

Selezionare quindi il campo di origine che si desidera aggiungere dalla struttura dello schema di origine visualizzata, quindi selezionare **[!UICONTROL Select]**.

![Lo schema di origine con &quot;paese&quot; selezionato come nuovo campo aggiuntivo.](../images/ui/mapping/source_field.png)

L’interfaccia di mappatura viene aggiornata con il campo sorgente selezionato e un campo di destinazione vuoto. Selezionare **[!UICONTROL Map target field]** per iniziare a mappare il nuovo campo di origine al relativo campo XDM di destinazione appropriato.
...
Per creare un campo calcolato, selezionare **[!UICONTROL New field type]**, quindi selezionare **[!UICONTROL Add calculated field]**

![Interfaccia di mappatura con il pulsante &quot;Aggiungi campo calcolato&quot; selezionato.](../images/ui/mapping/new_calculated_field.png)

Viene visualizzata la finestra **[!UICONTROL Create calculated field]**. Utilizza l’interfaccia per inserire i campi calcolati e fai riferimento alla finestra di dialogo a sinistra per i campi, le funzioni e gli operatori supportati.

| Scheda | Descrizione |
| --- | ----------- |
| [!UICONTROL Function] | Nella scheda Funzioni sono elencate le funzioni disponibili per la trasformazione dei dati. Per ulteriori informazioni sulle funzioni utilizzabili nei campi calcolati, leggere la guida sulle [funzioni di preparazione dati (mapper)](../functions.md). |
| [!UICONTROL Field] | Nella scheda Campi sono elencati i campi e gli attributi disponibili nello schema di origine. |
| [!UICONTROL Operator] | Nella scheda operatori sono elencati gli operatori disponibili per la trasformazione dei dati. |

![Interfaccia del campo calcolato](../images/ui/mapping/calculated_field.png)

Puoi aggiungere manualmente campi, funzioni e operatori utilizzando l’editor di espressioni al centro. Seleziona l’editor per iniziare a creare un’espressione. Al termine, selezionare **[!UICONTROL Save]** per continuare.
...
Per importare la mappatura da un flusso di dati esistente, selezionare **[!UICONTROL Import mappings]**, quindi selezionare **[!UICONTROL Import mapping from flow]**.
...
Quindi, utilizza la finestra a comparsa per individuare il flusso di dati di cui desideri importare la mappatura. Durante questo passaggio, puoi anche utilizzare la funzione di ricerca per isolare un flusso di dati specifico e recuperarne le mappature. Al termine, selezionare **[!UICONTROL Select]**.
...
Per importare la mappatura da un file esportato, selezionare **[!UICONTROL Import mappings]**, quindi selezionare **[!UICONTROL Import mapping from file]**.
...
Al termine, selezionare **[!UICONTROL Upload file]** e selezionare il file CSV aggiornato delle mappature. Attendere qualche istante per l&#39;elaborazione del sistema, quindi selezionare **[!UICONTROL Done]**.

![Finestra del modello di caricamento con un nuovo file caricato.](../images/ui/mapping/upload_successful.png)

>[!ENDTABS]

Al termine delle mappature, puoi selezionare **[!UICONTROL Finish]** e procedere al passaggio successivo per completare il flusso di dati.
