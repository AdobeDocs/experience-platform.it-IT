...
>[!NOTE] Quando viene creata per la prima volta una sandbox non contiene dati. Poiché ogni sandbox mantiene un proprio archivio dati isolato, deve anche acquisire i dati in modo indipendente.
...
Leggendo questo documento, hai introdotto i concetti fondamentali sulle sandbox in Experience Platform. Per i passaggi dettagliati su come gestire le sandbox, consultate la guida [](ui/overview.md) utente per l&#39;interfaccia utente o la guida [](./api/getting-started.md) per gli sviluppatori per l&#39;API.

Sebbene le sandbox costituiscano uno strumento prezioso per isolare gli ambienti della piattaforma per il team di sviluppo, potete anche gestire un controllo degli accessi più dettagliato utilizzando Adobe Admin Console. Per ulteriori informazioni, consulta la panoramica [sul controllo](../access-control/home.md) degli accessi.