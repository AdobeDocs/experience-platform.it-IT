...
| `decisioning.propositionDisplay` | Questo evento viene utilizzato quando il Web SDK invia automaticamente informazioni sugli elementi visualizzati in una pagina. Tuttavia, non è necessario questo tipo di evento se si includono già le informazioni di visualizzazione in altri modi, come con gli hit di pagina superiori e inferiori. Per la parte inferiore degli hit pagina, puoi scegliere qualsiasi tipo di evento desiderato. |
...
| `decisioning.propositionTrigger` | Gli eventi di questo tipo vengono archiviati nell’archivio locale dal Web SDK ma non vengono inviati ad Edge Network. Ogni volta che un set di regole viene soddisfatto, un evento viene generato e memorizzato nell’archiviazione locale (se tale impostazione è abilitata). |
