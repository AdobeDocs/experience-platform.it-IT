...
Una volta che hai [!DNL Schema Editor] apri, i controlli per aggiungere campi vengono visualizzati nell’area di lavoro. Tali controlli vengono visualizzati accanto al nome dello schema, nonché a tutti i campi di tipo oggetto definiti nella classe o nel gruppo di campi selezionati.
...
| [!UICONTROL Nome visualizzato] | Nome visualizzato del campo. Questo è il nome che verrà utilizzato per rappresentare il campo nell’area di lavoro dell’Editor di schema. Il nome del campo può essere modificato nel nome visualizzato utilizzando [interruttore nome visualizzato](../resources/schemas.md#display-name-toggle). |
