...
[!UICONTROL Privacy/Personalization/Marketing Preferences (Consents)] (in seguito denominato  [!DNL Privacy & Consents] mixin) è un mixin standard per la  [[!DNL XDM Individual Profile] classe](../../classes/individual-profile.md), utilizzato per acquisire le informazioni sul consenso e sulle preferenze dei clienti.

>[!NOTE]
>
>Poiché questo mixin è compatibile solo con [!DNL XDM Individual Profile], non può essere utilizzato per gli schemi [!DNL XDM ExperienceEvent]. Se desideri includere i dati di consenso e preferenza nello schema Evento esperienza, aggiungi il tipo di dati [[!UICONTROL Consent for Privacy, Personalization and Marketing Preferences]](../../data-types/consents.md) allo schema utilizzando invece un mixin [personalizzato](../../ui/resources/mixins.md#create).

## Struttura mista {#structure}

>[!IMPORTANT]
>
>Il mixin [!DNL Consents & Preferences] è progettato per coprire una serie di casi d’uso relativi al consenso e alla gestione delle preferenze. Di conseguenza, questo documento descrive l&#39;uso dei campi del mixin in termini generali e formula solo suggerimenti su come interpretare l&#39;uso di questi campi. Consulta il team legale per la privacy per allineare la struttura del mixin a come la tua organizzazione interpreta e presenta ai clienti queste scelte di consenso e preferenza.

Il mixin [!DNL Consents & Preferences] fornisce diversi campi utilizzati per acquisire le informazioni di **consenso** e **preferenza**.
...
La schermata seguente mostra come la struttura del mixin è rappresentata nell’interfaccia utente della piattaforma:
...
Il seguente JSON mostra un esempio del tipo di dati che il mixin [!DNL Consents & Preferences] può elaborare. Le informazioni sull&#39;uso specifico di ciascuno di questi campi sono fornite nelle sezioni seguenti.
...
Il consenso `adID` rappresenta il consenso del cliente circa la possibilità di utilizzare un ID inserzionista (IDFA o GAID) per collegare il cliente tra le app su questo dispositivo. Questo valore può essere configurato solo sotto lo spazio dei nomi `ECID` identity nella sezione `idSpecific` e non può essere impostato per altri namespace o a livello di utente per questo mixin.
...
## Acquisizione di dati tramite il mixin {#ingest}

Per utilizzare il mixin [!DNL Consents & Preferences] per acquisire i dati di consenso dai clienti, è necessario creare un set di dati basato su uno schema che contiene tale mixin.

Per informazioni su come assegnare i mixin ai campi, consulta l’esercitazione su [creazione di uno schema nell’interfaccia utente](http://www.adobe.com/go/xdm-schema-editor-tutorial-en) . Dopo aver creato uno schema contenente un campo con il mixin [!DNL Consents & Preferences], consulta la sezione relativa alla [creazione di un set di dati](../../../catalog/datasets/user-guide.md#create) nella guida utente del set di dati, seguendo i passaggi per creare un set di dati con uno schema esistente.

>[!IMPORTANT]
>
>Se desideri inviare i dati di consenso a [!DNL Real-time Customer Profile], è necessario creare uno schema abilitato [!DNL Profile] in base alla classe [!DNL XDM Individual Profile] che contiene il mixin [!DNL Consents & Preferences]. Anche il set di dati creato in base a tale schema deve essere abilitato per [!DNL Profile]. Fai riferimento alle esercitazioni collegate sopra per passaggi specifici relativi ai requisiti [!DNL Real-time Customer Profile] per schemi e set di dati.
>
...
Le sezioni seguenti forniscono informazioni di riferimento aggiuntive relative al mixin [!DNL Consents & Preferences].
...
Per visualizzare lo schema completo del mixin [!DNL Consents & Preferences], fai riferimento al [repository XDM ufficiale](https://github.com/adobe/xdm/blob/master/components/datatypes/consent-preferences.schema.json).
