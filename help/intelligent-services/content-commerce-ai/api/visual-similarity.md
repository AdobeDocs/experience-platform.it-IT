...
title: Somiglianza visiva nell’API di IA per l’analisi dei contenuti e del commercio
...
Una risposta corretta restituisce un `response` che contiene un `feature_value` e `feature_name` per ciascuna delle immagini visivamente simili presenti nel catalogo.
