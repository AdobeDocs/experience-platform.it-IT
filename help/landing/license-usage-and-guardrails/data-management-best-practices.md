...
Experience Platform è composto principalmente da due archivi di dati: la [!DNL data lake] e l&#39;archivio profili.
...
* La somma di tutti i dati di produzione archiviati in Adobe Real-time Customer Data Platform (ad esempio, Profilo cliente in tempo reale e Servizio identità) in qualsiasi momento, divisa per [!DNL Addressable Audience];
* La somma di tutti i dati archiviati in Platform (compresi, tra l’altro, [!DNL data lake], Profilo cliente in tempo reale e servizio Identity) in qualsiasi momento e in qualsiasi momento e nei dati che trasmetti (anziché archiviarli) in Platform negli ultimi 12 mesi, suddivisi per [!DNL Addressable Audience].
...
| --- | --- |
| Filtro sorgente Adobe Audience Manager | Quando crei una connessione sorgente Adobe Audience Manager, puoi scegliere i segmenti e le caratteristiche da inserire nella [!DNL data lake] e Profilo cliente in tempo reale, anziché acquisire i dati Audienci Manager nella loro interezza. Consulta la guida su [creazione di una connessione sorgente Audience Manager](../../sources/tutorials/ui/create/adobe-applications/audience-manager.md) per ulteriori informazioni. |
