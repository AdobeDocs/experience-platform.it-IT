...
Affinché l’SDK invii dati ad Experience Platform, devi prima configurare un datastream. Nell’interfaccia utente Raccolta dati, seleziona **[!UICONTROL Datastreams]** nel menu di navigazione a sinistra.

Dopo aver creato un nuovo datastream o aver selezionato uno esistente da modificare, seleziona il pulsante di attivazione accanto a **[!UICONTROL Adobe Experience Platform]**. Quindi, utilizza i valori elencati di seguito per completare il modulo.
...
| [!UICONTROL Set di dati evento] | Un set di dati [!DNL XDM ExperienceEvent] che prevedi di inviare dati evento all’utilizzo dell’SDK. Anche se devi fornire un set di dati evento per creare un archivio dati Platform, tieni presente che i dati di consenso inviati tramite eventi non sono rispettati nei flussi di lavoro di applicazione a valle. |
| [!UICONTROL Set di dati del profilo] | Il set di dati abilitato [!DNL Profile] con i campi di consenso dei clienti creati [before](#prerequisites). |
...
>[!IMPORTANT]
>
>Il comando `setConsent` aggiorna solo i dati direttamente nell&#39;archivio profili e non invia dati al Data Lake.
...
Dopo aver effettuato con successo le chiamate `setConsent` con l’SDK, puoi utilizzare il visualizzatore del profilo nell’interfaccia utente di Platform per verificare se i dati sono di destinazione nell’archivio profili. Per ulteriori informazioni, consulta la sezione su [come visualizzare i profili per identità](../../../profile/ui/user-guide.md#browse-identity) .
