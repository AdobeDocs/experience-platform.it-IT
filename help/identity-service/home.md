...
Le relazioni di identità definite e mantenute da Identity Service sono sfruttate dal profilo cliente in tempo reale per creare un quadro completo di un cliente e delle sue interazioni con il tuo marchio. Per ulteriori informazioni, consulta la panoramica [Profilo cliente](../profile/home.md)in tempo reale.
...
Esempi di implementazioni di Identity Service:
...
Experience Platform affronta questa sfida attraverso due concetti: spazi dei nomi [di identità](#identity-namespaces) e grafici [di](#identity-graphs)identità.
...
Comprendere il cliente su più dispositivi e canali inizia riconoscendolo in ciascun canale. Adobe Experience Platform raggiunge questo obiettivo utilizzando gli spazi dei nomi delle identità.
...
Tutti i grafici dell&#39;identità del cliente vengono gestiti collettivamente e aggiornati da Identity Service in tempo quasi reale, in risposta all&#39;attività del cliente.
...
## Fornitura di dati di identità a Servizio identità

Questa sezione illustra come vengono elaborati i dati forniti ad Adobe Experience Platform prima di essere utilizzati da Identity Service per creare un grafico dell&#39;identità per ciascun cliente.
...
Sebbene Experience Platform offra diversi spazi dei nomi standard, potrebbe essere necessario creare spazi dei nomi aggiuntivi per classificare correttamente le identità. Per ulteriori informazioni, consultate la sezione sulla [visualizzazione e la creazione di spazi dei nomi per la vostra organizzazione](./namespaces.md) nella panoramica dello spazio dei nomi delle identità.
...
### Includi dati di identità in Experience Data Model (XDM)

Come framework standard con cui la piattaforma organizza i dati dei clienti, Experience Data Model (XDM) consente di condividere e comprendere i dati tra la piattaforma Experience e altri servizi che interagiscono con la piattaforma. Per ulteriori informazioni, vedere la panoramica [del sistema](../xdm/home.md)XDM.
...
Ad esempio, etichettando i campi dei numeri di telefono come campi di identità, il servizio identità traccia automaticamente le relazioni con gli altri utenti che utilizzano lo stesso numero di telefono.

>[!NOTE] Lo spazio dei nomi delle identità risultanti viene fornito al momento in cui il campo è etichettato.

### Configurare un set di dati per il servizio identità

Durante il processo di assimilazione in streaming, Identity Service estrae automaticamente i dati di identità dai dati dei record e delle serie temporali. Tuttavia, prima di poter essere assimilati, i dati devono essere attivati per il servizio identità. Per ulteriori informazioni, consulta l’esercitazione sulla [configurazione di un set di dati per il profilo cliente e il servizio identità in tempo reale mediante l’utilizzo delle API](../profile/tutorials/dataset-configuration.md) .

### Inviare dati a Servizio identità

Identity Service utilizza i dati conformi a XDM inviati a Experience Platform tramite [caricamento](../ingestion/batch-ingestion/overview.md) batch o [assimilazione](../ingestion/streaming-ingestion/overview.md)in streaming.

## Governance dei dati

Adobe Experience Platform è stato creato tenendo presente la privacy e include un framework di governance dei dati per proteggere i dati PII dei clienti. I dati di identità nello spazio dei nomi &quot;email&quot; o &quot;phone&quot; sono crittografati per impostazione predefinita, ma per garantire che i dati riservati siano crittografati prima che vengano memorizzati, le etichette di utilizzo dei dati possono essere applicate ai dati durante l&#39;assimilazione o al loro arrivo in Piattaforma. Per ulteriori informazioni, consulta la panoramica [sulla governance dei](../data-governance/home.md)dati.

## Passaggi successivi

Ora che hai compreso i concetti chiave di Servizio identità e il suo ruolo all’interno della Piattaforma esperienza, puoi iniziare a imparare a utilizzare il grafico dell’identità utilizzando l’API [del servizio](./api/getting-started.md)identità.