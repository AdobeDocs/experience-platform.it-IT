...
È possibile restituire un elenco di tutti i processi di esportazione per una particolare organizzazione IMS eseguendo una richiesta di GET al `export/jobs` endpoint. La richiesta supporta anche i parametri di query `limit` e `offset`, come illustrato di seguito.
...
La risposta include una `records` oggetto contenente i processi di esportazione creati dall’organizzazione IMS.
