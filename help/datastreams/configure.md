...
Se si abilita uno dei campi sopra indicati per la raccolta dati, assicurarsi di impostare correttamente la proprietà dell&#39;array [`context`](/help/collection/js/commands/configure/context.md) durante la configurazione del Web SDK.
...
| [!UICONTROL First Party ID Cookie] | Quando è abilitata, questa impostazione comunica alla rete Edge di fare riferimento a un cookie specificato durante la ricerca di un [ID dispositivo di prime parti](/help/collection/use-cases/identity/first-party-device-ids.md), anziché cercare questo valore nella mappa di identità.<br><br>Quando si abilita questa impostazione, è necessario fornire il nome del cookie che deve memorizzare l&#39;ID. |
...
| [!UICONTROL Third Party ID Sync Container ID] | ID numerico del contenitore da utilizzare per la sincronizzazione dell’ID di terze parti.<br><br>**Nota:** lo stream di dati fa riferimento all&#39;ID contenitore predefinito di Audience Manager, che è 0. Se disponi di più ID contenitore di sincronizzazione ID di Audience Manager, rivolgiti al tuo consulente Audience Manager per identificare e risolvere correttamente la configurazione di sincronizzazione ID. |
...
Questo servizio controlla se e come i dati vengono inviati ad Adobe Analytics.
...
Per ulteriori informazioni, consulta [Implementare Adobe Analytics con Edge Network](https://experienceleague.adobe.com/it/docs/analytics/implementation/aep-edge/overview) nella guida all&#39;implementazione di Analytics.
