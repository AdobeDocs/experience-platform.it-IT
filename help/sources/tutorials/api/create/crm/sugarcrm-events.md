...
# (Beta) Crea una connessione sorgente e un flusso di dati per [!DNL SugarCRM Events] utilizzo dell’API del servizio Flusso
...
Il seguente tutorial illustra i passaggi necessari per creare un [!DNL SugarCRM Events] connessione sorgente e crea un flusso di dati per portare [[!DNL SugarCRM]](https://www.sugarcrm.com/) Dati degli eventi da inviare a Adobe Experience Platform utilizzando [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/).
...
## Connetti [!DNL SugarCRM Events] alla piattaforma utilizzando [!DNL Flow Service] API

Di seguito sono descritti i passaggi da eseguire per autenticare il [!DNL SugarCRM] origine, crea una connessione di origine e crea un flusso di dati per portare i dati degli eventi in Experience Platform.
...
Per creare un ID di connessione di base, effettua una richiesta POST al `/connections` endpoint durante la fornitura del [!DNL SugarCRM Events] credenziali di autenticazione come parte del corpo della richiesta.
...
| `{SOURCE_PARAMS}` | Definisce i parametri per il file sorgente da portare a Platform. Per recuperare il formato accettato per `{SOURCE_PARAMS}`, è necessario codificare l&#39;intera stringa in base64. <br> [!DNL SugarCRM Events] non richiede alcun payload. Il valore per `{SOURCE_PARAMS}` viene passato come `{}`, codificato in base64 equivale a `e30=` come mostrato di seguito. |
...
La richiesta seguente crea una connessione sorgente per [!DNL SugarCRM Events]:
...
| `data.format` | Il formato del [!DNL SugarCRM Events] i dati che desideri acquisire. Attualmente, l’unico formato di dati supportato è `json`. |
...
| `data.format` | Il formato del [!DNL SugarCRM Events] i dati che desideri acquisire. |
...
L’ultimo passaggio per importare dati da [!DNL SugarCRM Events] In Platform è necessario creare un flusso di dati. A questo punto sono stati preparati i seguenti valori obbligatori:
