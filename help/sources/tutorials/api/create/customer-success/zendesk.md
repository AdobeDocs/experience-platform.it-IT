...
In caso di esito positivo, la risposta restituisce la struttura del file su cui è stata eseguita la query. Nell&#39;esempio seguente nel payload ``data[]`` viene visualizzato un solo record, tuttavia potrebbero essere presenti più record.
...
| `data.format` | Formato dei dati [!DNL Zendesk] che si desidera acquisire. Attualmente, l&#39;unico formato di dati supportato è `json`. |
