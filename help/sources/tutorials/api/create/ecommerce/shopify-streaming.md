---
title: Creare una connessione sorgente di streaming e un flusso di dati per i dati di Shopify utilizzando l’API del servizio Flusso
...
# Creare una connessione e un flusso di dati di origine in streaming per [!DNL Shopify] dati utilizzando l’API del servizio Flusso

>[!NOTE]
>
>Il [!DNL Shopify] l&#39;origine di streaming è in versione beta. Leggi le [panoramica sulle origini](../../../../home.md#terms-and-conditions) per ulteriori informazioni sull’utilizzo di fonti etichettate beta.

Il seguente tutorial illustra i passaggi necessari per creare una connessione a un’origine di streaming e un flusso di dati da cui trasmettere i dati [[!DNL Shopify]](https://www.shopify.com/) a Adobe Experience Platform utilizzando [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/).
...
## Flusso [!DNL Shopify] dati a Platform tramite l’API del servizio Flusso

Di seguito sono descritti i passaggi da eseguire per creare una connessione di origine e un flusso di dati per lo streaming [!DNL Shopify] su Platform.
...
| `data.format` | Il formato del [!DNL Shopify] i dati che desideri acquisire. Attualmente, l’unico formato di dati supportato è `json`. |
...
La richiesta seguente crea una connessione di destinazione per [!DNL Shopify]:
...
| `data.format` | Il formato del [!DNL Shopify] dati da inserire in Platform. |
...
L’ultimo passaggio per importare dati da [!DNL Shopify] In Platform è necessario creare un flusso di dati. A questo punto sono stati preparati i seguenti valori obbligatori:
...
| Proprietà | Descrizione |
| --- | --- |
| `name` | Nome del flusso di dati. Assicurati che il nome del flusso di dati sia descrittivo, in quanto puoi utilizzarlo per cercare informazioni su di esso. |
...
Aggiorna i dettagli del flusso di dati, ad esempio il nome e la descrizione, nonché la pianificazione di esecuzione e i set di mappatura associati, effettuando una richiesta PATCH al `/flows` endpoint del [!DNL Flow Service] e fornire l’ID del flusso di dati. Quando effettui una richiesta PATCH, devi fornire il codice univoco del flusso di dati `etag` nel `If-Match` intestazione. Per esempi API completi, consulta la guida su [aggiornamento dei flussi di dati di origine tramite l’API](https://experienceleague.adobe.com/docs/experience-platform/sources/api-tutorials/update-dataflows.html)
