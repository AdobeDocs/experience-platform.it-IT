...
# Creare una connessione di origine e un flusso di dati per [!DNL SAP Commerce] utilizzo dell’API del servizio Flusso
...
Le sezioni seguenti forniscono informazioni aggiuntive che è necessario conoscere per connettersi correttamente a [!DNL SAP Commerce] utilizzando [!DNL Flow Service] API.
...
Per ulteriori informazioni su queste credenziali, fare riferimento a [[!DNL SAP Commerce] documentazione](https://help.sap.com/docs/CLOUD_TO_CASH_OD/987aec876092428f88162e438acf80d6/c5fcaf96daff4c7a8520188e4d8a1843.html).

## Connetti [!DNL SAP Commerce] alla piattaforma utilizzando [!DNL Flow Service] API
...
Per creare un ID di connessione di base, effettua una richiesta POST al `/connections` endpoint durante la fornitura del [!DNL SAP Commerce] credenziali di autenticazione come parte del corpo della richiesta.
...
| `{SOURCE_PARAMS}` | Definisce i parametri per il file sorgente da portare a Platform. Per recuperare il formato accettato per `{SOURCE_PARAMS}`, è necessario codificare l&#39;intera stringa in base64. <br> [!DNL SAP Commerce] supporta più API. A seconda del tipo di oggetto utilizzato, passare una delle seguenti operazioni: <ul><li>`customers`</li><li>`contacts`</li></ul> |

Il [!DNL SAP Commerce] La sorgente supporta più API. Di seguito è riportato un esempio di oggetto a seconda del tipo di oggetto che si sta sfruttando per la richiesta da inviare:
...
Per [!DNL SAP Commerce] Il valore di API cliente per `{SOURCE_PARAMS}` viene passato come `{"object_type":"customers"}`. Quando codificato in base64, equivale a `eyJvYmplY3RfdHlwZSI6ImN1c3RvbWVycyJ9` come mostrato di seguito.
...
Per [!DNL SAP Commerce] API dei contatti il valore per `{SOURCE_PARAMS}` viene passato come `{"object_type":"contacts"}`. Quando codificato in base64, equivale a `eyJvYmplY3RfdHlwZSI6ImNvbnRhY3RzIn0=` come mostrato di seguito.
...
La richiesta seguente crea una connessione sorgente per [!DNL SAP Commerce] dati dei clienti:
...
| `data.format` | Il formato del [!DNL SAP Commerce] i dati che desideri acquisire. Attualmente, l’unico formato di dati supportato è `json`. |
| `object_type` | [!DNL SAP Commerce] supporta più API. Per l’API dei clienti, la `object_type` il parametro deve essere impostato su `customers`. |
...
La richiesta seguente crea una connessione sorgente per [!DNL SAP Commerce] dati contatti:
...
| `connectionSpec.id` | ID della specifica di connessione corrispondente all&#39;origine. |
| `data.format` | Il formato del [!DNL SAP Commerce] i dati che desideri acquisire. Attualmente, l’unico formato di dati supportato è `json`. |
| `object_type` | [!DNL SAP Commerce] supporta più API. Per l&#39;API dei contatti, il `object_type` il parametro deve essere impostato su `contacts`. |
| `path` | Avrà lo stesso valore selezionato per *`object_type`*. |
...
| `data.format` | Il formato del [!DNL SAP Commerce] i dati che desideri acquisire. |
...
La richiesta seguente crea una mappatura per [!DNL SAP Commerce] Dati API dei clienti
...
La richiesta seguente crea una mappatura per [!DNL SAP Commerce] Dati API dei contatti
...
L’ultimo passaggio per importare dati da [!DNL SAP Commerce] In Platform è necessario creare un flusso di dati. A questo punto sono stati preparati i seguenti valori obbligatori:
