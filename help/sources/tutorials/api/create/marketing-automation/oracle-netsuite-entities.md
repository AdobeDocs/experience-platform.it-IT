...
# Creare una connessione di origine e un flusso di dati per [!DNL Oracle NetSuite Entities] utilizzo dell’API del servizio Flusso
...
Leggi il seguente tutorial per scoprire come estrarre contatti e dati dei clienti dal tuo [!DNL Oracle NetSuite Activities Entities] account a Adobe Experience Platform utilizzando [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/).
...
Le sezioni seguenti forniscono informazioni aggiuntive che è necessario conoscere per connettersi correttamente a [!DNL Oracle NetSuite Entities] utilizzando [!DNL Flow Service] API.
...
## Connetti [!DNL Oracle NetSuite Entities] alla piattaforma utilizzando [!DNL Flow Service] API

Di seguito sono descritti i passaggi da eseguire per autenticare il [!DNL Oracle NetSuite Entities] origine, crea una connessione di origine e crea un flusso di dati per portare in Experience Platform i dati di clienti e contatti.
...
Per creare un ID di connessione di base, effettua una richiesta POST al `/connections` endpoint durante la fornitura del [!DNL Oracle NetSuite Entities] credenziali di autenticazione come parte del corpo della richiesta.
...
| `{SOURCE_PARAMS}` | Definisce i parametri per il file sorgente da portare a Platform. Per recuperare il formato accettato per `{SOURCE_PARAMS}`, è necessario codificare l&#39;intera stringa in base64. <br> [!DNL Oracle NetSuite Entities] supporta il recupero dei dati dei clienti e dei contatti. A seconda del tipo di oggetto utilizzato, passare una delle seguenti operazioni: <ul><li>`customer` : recupera dati cliente specifici, inclusi dettagli quali nomi dei clienti, indirizzi e identificatori chiave.</li><li>`contact` : recupera i nomi dei contatti, le e-mail, i numeri di telefono ed eventuali campi personalizzati relativi ai contatti associati ai clienti.</li></ul> |

>[!BEGINTABS]

>[!TAB Cliente]

Per [!DNL Oracle NetSuite Entities], per recuperare i dati di contatto il valore per `{SOURCE_PARAMS}` viene passato come `{"object_type":"customer"}`. Quando codificato in base64, equivale a `eyAib2JqZWN0X3R5cGUiOiAiY3VzdG9tZXIifQ%3D%3D` come mostrato di seguito.
...
Per [!DNL Oracle NetSuite Entities], per recuperare i dati di contatto il valore per `{SOURCE_PARAMS}` viene passato come `{"object_type":"contact"}`. Quando codificato in base64, equivale a `eyAib2JqZWN0X3R5cGUiOiAiY29udGFjdCJ9` come mostrato di seguito.
...
La richiesta seguente crea una connessione sorgente per [!DNL Oracle NetSuite Entities]:

>[!BEGINTABS]

>[!TAB Cliente]

Quando si recuperano i dati dei clienti, `object_type` il valore della proprietà deve essere `customer`.
...
Quando si recuperano i dati di contatto, `object_type` il valore della proprietà deve essere `contact`.
...
| `data.format` | Il formato del [!DNL Oracle NetSuite Entities] i dati che desideri acquisire. Attualmente, l’unico formato di dati supportato è `json`. |
...
| `data.format` | Il formato del [!DNL Oracle NetSuite Entities] i dati che desideri acquisire. |
...
L’ultimo passaggio per importare dati da [!DNL Oracle NetSuite Entities] In Platform è necessario creare un flusso di dati. A questo punto sono stati preparati i seguenti valori obbligatori:
