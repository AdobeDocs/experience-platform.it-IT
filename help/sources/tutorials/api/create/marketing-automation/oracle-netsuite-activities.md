...
# Creare una connessione di origine e un flusso di dati per [!DNL Oracle NetSuite Activities] utilizzo dell’API del servizio Flusso
...
Leggi il seguente tutorial per scoprire come estrarre i dati di eventi dal tuo [!DNL Oracle NetSuite Activities] account a Adobe Experience Platform utilizzando [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/).
...
Le sezioni seguenti forniscono informazioni aggiuntive che è necessario conoscere per connettersi correttamente a [!DNL Oracle NetSuite Activities] utilizzando [!DNL Flow Service] API.
...
## Connetti [!DNL Oracle NetSuite Activities] alla piattaforma utilizzando [!DNL Flow Service] API

Segui la guida riportata di seguito per scoprire come autenticare il [!DNL Oracle NetSuite Activities] origine, crea una connessione di origine e crea un flusso di dati per portare i dati degli eventi in Experience Platform.
...
Per creare un ID di connessione di base, effettua una richiesta POST al `/connections` endpoint durante la fornitura del [!DNL Oracle NetSuite Activities] credenziali di autenticazione come parte del corpo della richiesta.
...
La richiesta seguente crea una connessione di base per [!DNL Oracle NetSuite Activities]:
...
| `auth.params.clientId` | Il valore ID client quando crei il record di integrazione. È possibile trovare il processo per creare un record di integrazione [qui](https://docs.oracle.com/en/cloud/saas/netsuite/ns-online-help/section_157771733782.html#procedure_157838925981). Il valore è una stringa di 64 caratteri simile a `7fce.....b42f`. |
| `auth.params.clientSecret` | Il valore ID client quando crei il record di integrazione. È possibile trovare il processo per creare un record di integrazione [qui](https://docs.oracle.com/en/cloud/saas/netsuite/ns-online-help/section_157771733782.html#procedure_157838925981). Il valore è una stringa di 64 caratteri simile a `5c98.....1b46`. |
| `auth.params.accessTokenUrl` | Il [!DNL NetSuite] URL token di accesso, simile a `https://{ACCOUNT_ID}.suitetalk.api.netsuite.com/services/rest/auth/oauth2/v1/token` dove sostituirai ACCOUNT_ID con il tuo [!DNL NetSuite] ID account. |
| `auth.params.accessToken` | Il valore del token di accesso viene generato alla fine di [Passaggio due](https://docs.oracle.com/en/cloud/saas/netsuite/ns-online-help/section_158081952044.html#Step-Two-POST-Request-to-the-Token-Endpoint) del [Flusso di concessione codice di autorizzazione OAuth 2.0](https://docs.oracle.com/en/cloud/saas/netsuite/ns-online-help/section_158074210415.html#OAuth-2.0-Authorization-Code-Grant-Flow) esercitazione. La scadenza dei token di accesso è valida solo per 60 minuti. il valore è una stringa di 1024 caratteri formattata come JSON Web Token (JWT) simile a `eyJr......f4V0`. |
...
| `{SOURCE_PARAMS}` | Definisce i parametri per il file sorgente da portare a Platform. Per recuperare il formato accettato per `{SOURCE_PARAMS}`, è necessario codificare l&#39;intera stringa in base64. <br> Il valore è vuoto per [!DNL Oracle NetSuite Activities].</li></ul> |
...
La richiesta seguente crea una connessione sorgente per [!DNL Oracle NetSuite Activities].
...
| `baseConnectionId` | ID connessione di base di [!DNL Oracle NetSuite Activities]. Questo ID è stato generato in un passaggio precedente. |
| `connectionSpec.id` | ID della specifica di connessione corrispondente all&#39;origine. |
| `data.format` | Il formato del [!DNL Oracle NetSuite Activities] i dati che desideri acquisire. Attualmente, l’unico formato di dati supportato è `json`. |
...
La richiesta seguente crea una connessione di destinazione per [!DNL Oracle NetSuite Activities]:
...
| `data.format` | Il formato del [!DNL Oracle NetSuite Activities] i dati che desideri acquisire. |
...
La richiesta seguente crea una mappatura per [!DNL DNL NetSuite Activities]
...
L’ultimo passaggio per importare dati da [!DNL Oracle NetSuite Activities] In Platform è necessario creare un flusso di dati. A questo punto sono stati preparati i seguenti valori obbligatori:
