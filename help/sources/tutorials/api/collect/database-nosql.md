---
keywords: Experience Platform;home;argomenti popolari;database database;database di terze parti
...
# Creare un flusso di dati per le origini del database utilizzando [!DNL Flow Service] API

Questa esercitazione descrive i passaggi per recuperare i dati da un’origine di database e portarli a Platform tramite [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/).
...
| `connectionSpec.id` | ID della specifica di connessione dell&#39;origine del database. Consulta la [Appendice](#appendix) per un elenco degli ID delle specifiche del database. |
...
| `transformations.params.deltaColum` | La colonna designata utilizzata per distinguere tra dati nuovi ed esistenti. I dati incrementali verranno acquisiti in base alla marca temporale della colonna selezionata. Il formato data supportato per `deltaColumn` è `yyyy-MM-dd HH:mm:ss`. Se si utilizza Azure Table Storage, il formato supportato per `deltaColumn` è `yyyy-MM-ddTHH:mm:ssZ`. |
...
Seguendo questa esercitazione, hai creato un connettore di origine per raccogliere i dati da un database su base pianificata. I dati in arrivo possono ora essere utilizzati da servizi Platform a valle come [!DNL Real-Time Customer Profile] e [!DNL Data Science Workspace]. Per ulteriori informazioni, consulta i seguenti documenti:
