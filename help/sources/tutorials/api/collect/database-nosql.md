...
Questa esercitazione richiede una connessione valida a un database e informazioni sul file che desideri inserire in Platform (compreso il percorso e la struttura del file). Se non disponi di queste informazioni, consulta l’esercitazione su [come esplorare un database utilizzando l’API del servizio di flusso](../explore/database-nosql.md) prima di provare questa esercitazione.
...
| `connectionSpec.id` | ID della specifica di connessione dell&#39;origine del database. Per un elenco degli ID delle specifiche del database, consulta l’ [Appendice](#appendix) . |
...
| `transformations.params.deltaColum` | Colonna designata utilizzata per distinguere tra dati nuovi ed esistenti. I dati incrementali verranno acquisiti in base al timestamp della colonna selezionata. Il formato di data supportato per `deltaColumn` è `yyyy-MM-dd HH:mm:ss`. Se utilizzi Azure Table Storage, il formato supportato per `deltaColumn` è `yyyy-MM-ddTHH:mm:ssZ`. |
...
Seguendo questa esercitazione, hai creato un connettore di origine per raccogliere dati da un database su base pianificata. I dati in arrivo possono ora essere utilizzati dai servizi Platform a valle, come [!DNL Real-time Customer Profile] e [!DNL Data Science Workspace]. Per ulteriori informazioni, consulta i seguenti documenti:
