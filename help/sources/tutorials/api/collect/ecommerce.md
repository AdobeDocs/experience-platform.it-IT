...
# Creare un flusso di dati per le origini di e-commerce utilizzando l&#39;API [!DNL Flow Service]

Questa esercitazione descrive i passaggi per recuperare i dati da un&#39;origine e-commerce e portarli a Platform utilizzando [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/).

>[!NOTE]
>
>* Per creare un flusso di dati, è necessario disporre già di un ID connessione di base valido con un’origine E-commerce. Se non disponi di questo ID, consulta la [panoramica origini](../../../home.md#ecommerce) per un elenco delle origini di e-commerce con cui puoi creare una connessione di base.
...
* [[!DNL Experience Data Model (XDM) System]](../../../../xdm/home.md): framework standardizzato in base al quale Experience Platform organizza i dati sull&#39;esperienza del cliente.
   ...
   * [API Schema Registry](../../../../xdm/api/getting-started.md): scopri come eseguire correttamente le chiamate all&#39;API Schema Registry. Ciò include `{TENANT_ID}`, il concetto di &quot;contenitori&quot; e le intestazioni necessarie per effettuare le richieste (con particolare attenzione all&#39;intestazione Accept e ai suoi possibili valori).
...
| `scheduleParams.frequency` | `frequency` in cui il flusso di dati raccoglierà i dati. I valori accettabili includono: `once`, `minute`, `hour`, `day` o `week`. |
| `scheduleParams.interval` | L’intervallo indica il periodo tra due esecuzioni consecutive del flusso. Il valore dell&#39;intervallo deve essere un numero intero diverso da zero. Non è necessario un intervallo quando `frequency` è impostato come `once` e deve essere maggiore o uguale a `15` per gli altri valori `frequency`. |
...
Seguendo questa esercitazione, hai creato un connettore di origine per raccogliere i dati di e-commerce su base pianificata. I dati in arrivo possono ora essere utilizzati da servizi [!DNL Platform] downstream come [!DNL Real-Time Customer Profile] e [!DNL Data Science Workspace]. Per ulteriori informazioni, consulta i seguenti documenti:
