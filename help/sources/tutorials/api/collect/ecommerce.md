---
keywords: Experience Platform;home;argomenti popolari;raccogliere dati eCommerce;dati eCommerce;home;popular topic;Collect eCommerce data;eCommerce data
solution: Experience Platform
title: Creare un flusso di dati per le origini di e-commerce utilizzando l’API del servizio Flusso
type: Tutorial
...
Questa esercitazione descrive i passaggi per recuperare i dati da un’origine e-commerce e portarli a Platform tramite [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/).
...
| -------- | ----------- |
| `baseConnectionId` | ID di connessione dell’origine e-commerce. |
| `params.path` | Percorso del file di origine. |
| `connectionSpec.id` | L’ID della specifica di connessione dell’origine e-commerce. |
...
| `transformations.params.mappingId` | L’ID di mappatura associato alla tua origine e-commerce. |
| `scheduleParams.startTime` | L’ora di inizio del flusso di dati in tempo epoca. |
| `scheduleParams.frequency` | Il `frequency` in cui il flusso di dati raccoglierà i dati. I valori accettabili includono: `once`, `minute`, `hour`, `day`, o `week`. |
| `scheduleParams.interval` | L’intervallo indica il periodo tra due esecuzioni consecutive del flusso. Il valore dell&#39;intervallo deve essere un numero intero diverso da zero. Non è necessario un intervallo quando `frequency` è impostato come `once` e deve essere maggiore o uguale a `15` per altri `frequency` valori. |
...
Seguendo questa esercitazione, hai creato un connettore di origine per raccogliere i dati di e-commerce su base pianificata. I dati in arrivo possono ora essere utilizzati da downstream [!DNL Platform] servizi quali [!DNL Real-Time Customer Profile] e [!DNL Data Science Workspace]. Per ulteriori informazioni, consulta i seguenti documenti:
