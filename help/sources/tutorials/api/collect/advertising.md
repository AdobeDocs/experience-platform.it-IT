...
title: Raccogliere dati pubblicitari utilizzando connettori sorgente e API
...
# Raccogliere dati pubblicitari utilizzando connettori sorgente e API

Questa esercitazione descrive i passaggi per recuperare i dati da un’applicazione pubblicitaria di terze parti e acquisirli in Adobe Experience Platform tramite i connettori sorgente e l’ [[!DNL Flow Service]](https://www.adobe.io/experience-platform-apis/references/flow-service/) API .

## Introduzione

Questa esercitazione richiede di disporre di informazioni sul file da importare in Platform, inclusi il percorso e la struttura del file. Se non disponi di queste informazioni, consulta l’esercitazione su [come esplorare un’applicazione pubblicitaria utilizzando l’ API del servizio di flusso](../../api/create/advertising/ads.md) prima di provare questa esercitazione.
...
| -------- | ----------- |
| `baseConnectionId` | ID di connessione univoco dell&#39;applicazione pubblicitaria a cui si accede. |
| `params.path` | Percorso del file di origine. |
| `connectionSpec.id` | ID delle specifiche di connessione associate all&#39;applicazione pubblicitaria. |
...
Un flusso di dati è responsabile della raccolta dei dati da origini e del loro inserimento in Platform. Per creare un flusso di dati, è innanzitutto necessario ottenere le specifiche del flusso di dati responsabili della raccolta dei dati pubblicitari.
...
L&#39;ultimo passo verso la raccolta dei dati pubblicitari è quello di creare un flusso di dati. A questo punto sono stati preparati i seguenti valori richiesti:
...
Seguendo questa esercitazione, hai creato un connettore di origine per raccogliere dati da un sistema pubblicitario su base pianificata. I dati in arrivo possono ora essere utilizzati dai servizi Platform a valle, come [!DNL Real-time Customer Profile] e [!DNL Data Science Workspace]. Per ulteriori informazioni, consulta i seguenti documenti:
