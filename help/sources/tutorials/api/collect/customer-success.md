...
title: Creare un flusso di dati per le origini di successo del cliente utilizzando l’API del servizio di flusso
...
Questa esercitazione descrive i passaggi per recuperare i dati da un’origine di successo del cliente e portarli a Platform utilizzando [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/).
...
Per tutti i connettori basati su tabella, imposta il valore `tabular`.
...
| `connectionSpec.id` | ID delle specifiche di connessione associato al sistema di successo cliente di terze parti specifico. Consulta la sezione [appendice](#appendix) per un elenco degli ID delle specifiche di connessione. |
...
Seguendo questa esercitazione, hai creato un connettore di origine per raccogliere i dati da un sistema di successo cliente su base pianificata. I dati in arrivo possono ora essere utilizzati da downstream [!DNL Platform] servizi quali [!DNL Real-time Customer Profile] e [!DNL Data Science Workspace]. Per ulteriori informazioni, consulta i seguenti documenti:
