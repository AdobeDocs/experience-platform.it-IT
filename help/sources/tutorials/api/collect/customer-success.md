...
# Raccogliere dati da un sistema di successo cliente utilizzando connettori sorgente e API

Questa esercitazione descrive i passaggi per recuperare i dati da un sistema di successo cliente di terze parti e acquisirli in [!DNL Platform] tramite i connettori sorgente e l’ API [[!DNL Flow Service]](https://www.adobe.io/apis/experienceplatform/home/api-reference.html#!acpdr/swagger-specs/flow-service.yaml).

## Introduzione

Questa esercitazione richiede l&#39;accesso a un sistema di successo cliente di terze parti tramite una connessione valida e informazioni sul file che desideri inserire in [!DNL Platform], compreso il percorso e la struttura del file. Se non si dispone di queste informazioni, vedere l&#39;esercitazione su [esplorare un database o un sistema NoSQL utilizzando l&#39;API del servizio di flusso](../explore/customer-success.md) prima di provare questa esercitazione.
...
* [[!DNL Batch ingestion]](../../../../ingestion/batch-ingestion/overview.md): L’API di acquisizione in batch consente di acquisire dati in  [!DNL Experience Platform] come file batch.
* [Sandbox](../../../../sandboxes/home.md):  [!DNL Experience Platform] fornisce sandbox virtuali che suddividono una singola  [!DNL Platform] istanza in ambienti virtuali separati per sviluppare e sviluppare applicazioni di esperienza digitale.

Le sezioni seguenti forniscono informazioni aggiuntive che sarà necessario conoscere per connettersi correttamente a un sistema di successo cliente utilizzando l’ [!DNL Flow Service] API .
...
Per tutti i connettori basati su tabelle, imposta il valore `tabular`.
...
| -------- | ----------- |
| `baseConnectionId` | ID di connessione univoco del sistema di successo cliente di terze parti a cui stai accedendo. |
| `params.path` | Percorso del file di origine. |
| `connectionSpec.id` | ID delle specifiche di connessione associato al sistema di successo cliente di terze parti specifico. Per un elenco degli ID delle specifiche di connessione, consulta l’ [appendice](#appendix) . |
...
Un flusso di dati è responsabile della raccolta dei dati da origini e del loro inserimento in Platform. Per creare un flusso di dati, devi prima ottenere le specifiche del flusso di dati eseguendo una richiesta GET all’API del servizio di flusso. Le specifiche del flusso di dati sono responsabili della raccolta di dati da un sistema di successo cliente di terze parti.
...
Seguendo questa esercitazione, hai creato un connettore di origine per raccogliere i dati da un sistema di successo cliente su base pianificata. I dati in entrata possono ora essere utilizzati dai servizi a valle [!DNL Platform] come [!DNL Real-time Customer Profile] e [!DNL Data Science Workspace]. Per ulteriori informazioni, consulta i seguenti documenti:
