---
keywords: Experience Platform;home;argomenti popolari;raccogliere dati protocollo;dati protocollo;home;popular topic;Collect protocol data;protocol data
solution: Experience Platform
title: Creare un flusso di dati per le origini dei protocolli utilizzando l’API del servizio Flusso
type: Tutorial
...
# Creare un flusso di dati per le origini dei protocolli Utilizzando [!DNL Flow Service] API

Questa esercitazione descrive i passaggi per recuperare i dati da un’origine di protocolli e portarli a Platform tramite [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/).
...
| -------- | ----------- |
| `baseConnectionId` | ID di connessione dell&#39;applicazione dei protocolli |
| `params.path` | Percorso del file di origine. |
| `connectionSpec.id` | ID della specifica di connessione dell&#39;applicazione dei protocolli. |
...
Un flusso di dati è responsabile della raccolta dei dati dalle origini e della loro introduzione in Platform. Per creare un flusso di dati, devi innanzitutto ottenere le specifiche del flusso di dati responsabili della raccolta dei dati dall’applicazione dei protocolli.
...
| `transformations.params.deltaColum` | La colonna designata utilizzata per distinguere tra dati nuovi ed esistenti. I dati incrementali verranno acquisiti in base alla marca temporale della colonna selezionata. Il formato supportato per `deltaColumn` quando si utilizza OData generica è `yyyy-MM-ddTHH:mm:ssZ`. |
...
Seguendo questa esercitazione, è stato creato un connettore di origine per raccogliere i dati da un&#39;applicazione di protocolli su base pianificata. I dati in arrivo possono ora essere utilizzati da servizi Platform a valle come [!DNL Real-Time Customer Profile] e [!DNL Data Science Workspace]. Per ulteriori informazioni, consulta i seguenti documenti:
