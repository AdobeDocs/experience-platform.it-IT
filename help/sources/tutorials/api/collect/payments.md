...
# Crea un flusso di dati per le origini di pagamento utilizzando [!DNL Flow Service] API

Questa esercitazione descrive i passaggi per recuperare i dati da un’origine dei pagamenti e portarli a Platform utilizzando [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/).

>[!NOTE]
>
>Per creare un flusso di dati, è necessario disporre già di un valore valido [!DNL PayPal] ID connessione base. Se non disponi di questo ID, consulta i passaggi su come [creare un [!DNL PayPal] connessione di base](../create/payments/paypal.md) prima di provare a eseguire questa esercitazione.
...
Seguendo questa esercitazione, hai creato un connettore di origine per raccogliere i dati da un&#39;applicazione di pagamento su base pianificata. I dati in arrivo possono ora essere utilizzati dai servizi della piattaforma a valle, come [!DNL Real-time Customer Profile] e [!DNL Data Science Workspace]. Per ulteriori informazioni, consulta i seguenti documenti:
