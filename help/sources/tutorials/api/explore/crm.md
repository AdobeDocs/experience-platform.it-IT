...
Per esplorare il sistema CRM utilizzando le API di [!DNL Experience Platform], è necessario disporre di un ID di connessione valido. Se non disponi già di una connessione per il sistema CRM con cui desideri lavorare, puoi crearne una tramite le seguenti esercitazioni:
...
Utilizzando l’ID di connessione per il sistema CRM, puoi esplorare le tabelle di dati eseguendo le richieste di GET. Utilizzare la seguente chiamata per trovare il percorso della tabella da controllare o acquisire in [!DNL Experience Platform].
...
Una risposta corretta è un array di tabelle da al sistema di gestione delle relazioni con i clienti. Individuare la tabella che si desidera inserire in [!DNL Experience Platform] e prendere nota della relativa proprietà `path`, in quanto è necessario fornirla nel passaggio successivo per esaminarne la struttura.
...
Per controllare la struttura di una tabella dal sistema CRM in uso, eseguire una richiesta GET specificando il percorso di una tabella come parametro di query.
