...
Un flusso di dati è un’attività pianificata che recupera e acquisisce dati da un’origine a un set di dati in Adobe Experience Platform. Questo tutorial illustra come creare un flusso di dati per un’origine di consenso e preferenze utilizzando l’interfaccia utente di Platform.

>[!NOTE]
>
>* Per creare un flusso di dati, è necessario disporre già di un account autenticato con [!DNL OneTrust Integration] sorgente. Guarda il tutorial su [creazione di [!DNL OneTrust Integration] connessione sorgente nell’interfaccia utente](../../ui/create/consent-and-preferences/onetrust.md) per ulteriori informazioni.
...
Dopo aver creato il consenso e le preferenze dell’account di origine, il **[!UICONTROL Aggiungi dati]** viene visualizzato un passaggio che fornisce un’interfaccia per esplorare la gerarchia delle tabelle dell’account di consenso e preferenze.
...
>[!IMPORTANT]
>
>Non è possibile mappare valori di coppie di chiavi dinamiche come oggetto da [!DNL OneTrust] a Platform e deve specificare tali chiavi nello schema di destinazione per mappare i dati durante l’acquisizione.
...
Seguendo questa esercitazione, hai creato correttamente un flusso di dati per portare i dati dall’origine del consenso e delle preferenze a Platform. I dati in arrivo possono ora essere utilizzati da downstream [!DNL Platform] servizi quali [!DNL Real-Time Customer Profile] e [!DNL Data Science Workspace]. Per ulteriori informazioni, consulta i seguenti documenti:
