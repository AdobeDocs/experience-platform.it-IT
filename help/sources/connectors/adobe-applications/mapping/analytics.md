...
| Feed dati | Campo XDM | Tipo XDM | Descrizione |
| — | — | — |
| `post_evar1`<br/>`[...]`<br/>`post_evar250` | `_experience.analytics.customDimensions.`<br/>`eVars.eVar1`<br/>`[...]`<br/>`_experience.analytics.customDimensions.`<br/>`eVars.eVar250` | stringa | eVar di analisi personalizzate. Ogni organizzazione può utilizzare le eVar in modo diverso. |
...
| `post_first_hit_page_url` | `_experience.analytics.endUser.`<br/>`firstWeb.webPageDetails.URL` | stringa | URL della prima pagina raggiunta dal visitatore. |
...
| `post_pagename` | `web.webPageDetails.pageViews.value` | stringa | È uguale a 1 sugli hit con un nome di pagina. È simile alla metrica Visualizzazioni pagina di Adobe Analytics. |
...
| `geo_city` | `placeContext.geo.city` | stringa | Nome della città dell&#39;hit. Si basa sull’indirizzo IP dell’hit. |
...
| `geo_region` | `placeContext.geo.stateProvince` | stringa | Nome dello stato o dell&#39;area dell&#39;hit. Si basa sull’indirizzo IP dell’hit. |
| `geo_zip` | `placeContext.geo.postalCode` | stringa | Codice postale dell&#39;hit. Si basa sull’indirizzo IP dell’hit. |
| `os` | `_experience.analytics.environment.`<br/>`operatingSystemID` | numero intero | ID numerico che rappresenta il sistema operativo del visitatore. Si basa sulla colonna user_agent. |
| `search_page_num` | `search.pageDepth` | numero intero | Questa variabile viene utilizzata dalla dimensione Classificazione di tutte le pagine di ricerca e indica in quale pagina dei risultati di ricerca il sito | è apparso prima che l&#39;utente facesse clic sul sito. |
...
| `visit_search_page_num` | `_experience.analytics.session.`<br/>`search.pageDepth` | numero intero | Il nome della prima pagina della visita. |
| `post_prop1`<br/>`[...]`<br/>`post_prop75` | `_experience.analytics.customDimensions.`<br/>`listprops.prop1`<br/>`[...]`<br/>`_experience.analytics.customDimensions.`<br/>`listprops.prop75` | Oggetto | Proprietà personalizzate di Analytics, configurate come proprietà elenco. Contiene un elenco delimitato di valori. |
| `post_hier1`<br/>`[...]`<br/>`post_hier5` | `_experience.analytics.customDimensions.`<br/>`hierarchies.hier1`<br/>`[...]`<br/>`_experience.analytics.customDimensions.`<br/>`hierarchies.hier5` | Oggetto | Utilizzato da variabili gerarchiche e contiene un elenco delimitato di valori. | {values (matrice), delimitatore (stringa)} |
| `post_mvvar1`<br/>`[...]`<br/>`post_mvvar3` | `_experience.analytics.customDimensions.`<br/>`lists.list1.list[]`<br/>`[...]`<br/>`_experience.analytics.customDimensions.`<br/>`lists.list3.list[]` | matrice | Elenco di valori di variabile. Contiene un elenco delimitato di valori personalizzati, a seconda dell’implementazione. | {value (stringa), chiave (stringa)} |
...
| `post_page_type` | `web.webPageDetails.isErrorPage` | booleano | Utilizzato per popolare la dimensione Pagine non trovate. Questa variabile deve essere vuota o contenere &quot;ErrorPage&quot; |
...
| `post_product_list` | `productListItems[].items` | array | Elenco prodotti trasmesso tramite la variabile prodotti. | {SKU (stringa), quantità (numero intero), priceTotal (numero)} |
| `post_search_engine` | `search.searchEngine` | stringa | ID numerico che rappresenta il motore di ricerca che ha indirizzato il visitatore al sito. |
...
| `ref_type` | `web.webReferrer.type` | stringa | ID numerico che rappresenta il tipo di riferimento per l&#39;hit. |
| `visit_paid_search` | `_experience.analytics.session.`<br/>`search.isPaid` | booleano | Flag (1=a pagamento, 0=non a pagamento) che indica se il primo hit della visita proviene da un hit di ricerca a pagamento. |
...
| `visit_start_time_gmt` | `_experience.analytics.session.`<br/>`timestamp` | numero intero | Marca temporale del primo hit della visita in tempo UNIX®. |
