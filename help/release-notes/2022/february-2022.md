...
| ID dispositivo di prime parti | Ora è possibile inviare i propri ID dispositivo all’Edge Network di Adobe Experience Platform durante la raccolta dei dati dei clienti tramite Experience Platform Web SDK, fornendo una soluzione per le recenti restrizioni del browser sulla durata dei cookie di terze parti. Per ulteriori informazioni, consulta la guida su [ID dispositivo di prime parti](/help/collection/use-cases/identity/first-party-device-ids.md). |
