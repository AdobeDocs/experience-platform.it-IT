...
Nel dashboard dei profili viene visualizzata un’istantanea dei dati attributo (record) di cui dispone la tua organizzazione nell’archivio profili in Experienci Platform.
...
| Tipo di dati | [[!UICONTROL Informazioni sui file multimediali]](https://github.com/lidiaist/xdm/blob/master/components/datatypes/media.schema.json) | È stato aggiunto un `xdm:playhead` per acquisire il tempo della testina di riproduzione per un contenuto multimediale. È stata corretta la convalida del pattern per `xdm:videoSegment`. |
