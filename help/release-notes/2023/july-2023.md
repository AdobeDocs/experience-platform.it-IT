...
| Funzione | Descrizione |
| --- | --- |
| Pulsante di attivazione Editor di query ottimizzato | Il pulsante di attivazione Editor di query è stato ottimizzato per offrire maggiore accessibilità e per supportare più temi. Le impostazioni ottimizzate dell’editor consentono di abilitare il tema scuro o chiaro. Per ulteriori informazioni, consulta la [documentazione](../../query-service/ui/user-guide.md#enhanced-editor-toggle) dello strumento. |
...
Per ulteriori informazioni sul servizio Query Service, consulta la [Panoramica sul servizio query](../../query-service/home.md).
...
| --- | --- | --- |
| Classe | [[!UICONTROL XDM Individual Prospect Profile]](https://github.com/adobe/xdm/pull/1758/files) | Utilizza questa classe per inserire profili di potenziali clienti provenienti dai casi d’uso più complessi dei fornitori di dati per l’acquisizione clienti. |
| Gruppo di campi | [[!UICONTROL Enriched Event Segment Details]](https://github.com/adobe/xdm/pull/1754/files) | Un elenco di tipi di pubblico per i quali il profilo si qualifica al momento della raccolta dell’evento. |
...
| --- | --- | --- |
| Gruppo di campi | [[!UICONTROL MediaAnalytics Interaction Details]](https://github.com/adobe/xdm/pull/1756/files) | Il `meta:status` è stato aggiornato da sperimentale a `stable`. |
| Gruppo di campi | [[!UICONTROL Media Interaction Details]](https://github.com/adobe/xdm/pull/1756/files) | Il `meta:status` è stato aggiornato da `stable` a `deprecated`. |
| Tipo di dati | [[!UICONTROL Session details information]](https://github.com/adobe/xdm/pull/1756/files) | Il `meta:status` è stato aggiornato da `experimental` a `stable`. |
| Tipo di dati | [[!UICONTROL Qoe Data details information]](https://github.com/adobe/xdm/pull/1756/files) | Il `meta:status` è stato aggiornato da `experimental` a `stable`. |
| Tipo di dati | [[!UICONTROL Player state data information]](https://github.com/adobe/xdm/pull/1756/files) | Il `meta:status` è stato aggiornato da `experimental` a `stable`. |
| Tipo di dati | [[!UICONTROL Media event information]](https://github.com/adobe/xdm/pull/1756/files) | Il `meta:status` è stato aggiornato da `experimental` a `stable`. |
| Tipo di dati | [[!UICONTROL Media details information]](https://github.com/adobe/xdm/pull/1756/files) | Il `meta:status` è stato aggiornato da `experimental` a `stable`. |
| Tipo di dati | [[!UICONTROL Error details information]](https://github.com/adobe/xdm/pull/1756/files) | Il `meta:status` è stato aggiornato da `experimental` a `stable`. |
| Tipo di dati | [[!UICONTROL Error details information]](https://github.com/adobe/xdm/pull/1756/files) | Il `meta:status` è stato aggiornato da `stable` a `deprecated`. |
| Tipo di dati | [[!UICONTROL Custom metadata details information]](https://github.com/adobe/xdm/pull/1756/files) | Il `meta:status` è stato aggiornato da `experimental` a `stable`. |
| Tipo di dati | [[!UICONTROL Chapter details information]](https://github.com/adobe/xdm/pull/1756/files) | Il `meta:status` è stato aggiornato da `experimental` a `stable`. |
| Tipo di dati | [[!UICONTROL Advertising Pod details information]](https://github.com/adobe/xdm/pull/1756/files) | Il `meta:status` è stato aggiornato da `experimental` a `stable`. |
| Tipo di dati | [[!UICONTROL Advertising details information]](https://github.com/adobe/xdm/pull/1756/files) | Il `meta:status` è stato aggiornato da `experimental` a `stable`. |
| Estensione (gestione del percorso clienti) | [[!UICONTROL Domain]](https://github.com/adobe/xdm/pull/1756/files) | Campo `Domain` aggiunto a [!UICONTROL Adobe CJM ExperienceEvent - Message Profile Details] per registrare il dominio dell&#39;indirizzo e-mail del destinatario. |
| Estensione (gestione del percorso clienti) | [[!UICONTROL Channel's variant Name]](https://github.com/adobe/xdm/pull/1753/files) | Questo campo è stato aggiunto a [!UICONTROL AJO Entity Fields] per rappresentare il nome della variante di canale. |
| Estensione (Adobe Analytics) | [[!UICONTROL Context value]](https://github.com/adobe/xdm/pull/1761/files) | `Context value` è stato aggiunto a [!UICONTROL `Adobe Analytics ExperienceEvent Full Extension`]. |
