...
| Nome alias per statistiche calcolate | È ora possibile fornire un nome di alias per fare riferimento in modo descrittivo ai risultati delle statistiche calcolate nelle query SQL. Per informazioni su questo e su altri aggiornamenti del comando CALCOLA STATISTICHE, consulta la documentazione. Per ulteriori informazioni, consulta la [documentazione](../../query-service/essential-concepts/dataset-statistics.md#alias-name). |
