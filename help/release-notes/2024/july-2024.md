...
- [!BADGE Disponibilità Limitata]{type=Informative} [Composizione Pubblico Federato](#federated-audience-composition)
...
| --- | --- | --- |
| SDK per web | Tracciare automaticamente le interazioni della proposta | Ora puoi utilizzare la proprietà `autoCollectPropositionInteractions` in Web SDK per determinare se Web SDK deve raccogliere automaticamente le interazioni della proposta. Per ulteriori informazioni, consulta la documentazione [`autoCollectPropositionInteractions`](/help/collection/js/commands/configure/autocollectpropositioninteractions.md). |
