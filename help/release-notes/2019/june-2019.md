...
description: Note sulla versione di giugno 2019 per Adobe Experience Platform.
...
| Notebook | Ambiente personalizzato per data scientist/data engineer basato su [!DNL Jupyter Notebooks] per consentire loro di preparare dati, estrarre funzionalità e sviluppare modelli ML con un elenco curato di librerie e framework di apprendimento automatico popolari. |
...
| Funzione | Descrizione |
| -----------| ---------- |
| Editor delle query | Utilizza uno strumento basato su Web per scrivere, convalidare, testare ed eseguire query. Include una console per informazioni dettagliate sull’esecuzione delle query e la possibilità di visualizzare in anteprima i risultati delle query. |
