...
L’API di acquisizione in batch di Adobe Experience Platform consente di acquisire dati in Platform come file batch. I dati da acquisire possono essere dati di profilo provenienti da un file flat (ad esempio un file Parquet) o dati conformi a uno schema noto nel [!DNL Experience Data Model] (XDM).

La [Riferimento API per l’acquisizione in batch](https://developer.adobe.com/experience-platform-apis/references/batch-ingestion/) fornisce informazioni aggiuntive su queste chiamate API.
