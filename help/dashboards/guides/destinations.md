...
## [!UICONTROL Destinations] dati dashboard {#destinations-dashboard-data}
...
## Esplora il dashboard [!UICONTROL Destinations] {#explore}

Per passare al dashboard delle destinazioni all&#39;interno dell&#39;interfaccia utente di Experience Platform, seleziona **[!UICONTROL Destinations]** nella barra a sinistra, quindi seleziona la scheda **[!UICONTROL Overview]** per visualizzare il dashboard.

La data e l&#39;ora dello snapshot più recente vengono visualizzate nella parte superiore di [!UICONTROL Overview] accanto al menu a discesa di destinazione. Tutti i dati del widget sono accurati a partire da quella data e ora. Il timestamp dell’istantanea viene fornito in UTC; non si trova nel fuso orario del singolo utente o organizzazione.

>[!NOTE]
>
>Se la tua organizzazione ha poca esperienza con Experience Platform e non dispone ancora di destinazioni attive, la dashboard Destinazioni e la scheda [!UICONTROL Overview] non sono visibili. Selezionando [!UICONTROL Destinations] dal menu di navigazione a sinistra viene invece visualizzata la scheda [!UICONTROL Catalog]. Per ulteriori informazioni sulla scheda [!UICONTROL Catalog], fare riferimento alla guida dell&#39;area di lavoro [[!UICONTROL Destinations]](../../destinations/ui/destinations-workspace.md).
...
### Modifica il dashboard [!UICONTROL Destinations] {#modify}

Selezionare **[!UICONTROL Modify dashboard]** per modificare l&#39;aspetto del dashboard delle destinazioni. Le modifiche al dashboard sono per utente e non a livello di organizzazione. Potete spostare, aggiungere, ridimensionare e rimuovere widget dal dashboard e accedere alla libreria di widget per personalizzare il dashboard. Dalla libreria dei widget, puoi esplorare i widget disponibili e creare widget personalizzati per la tua organizzazione.
...
Il widget **[!UICONTROL Most used destinations]** visualizza le destinazioni principali della tua organizzazione in base al numero di tipi di pubblico mappati, all&#39;ultima istantanea. Questa classificazione fornisce ad insight le destinazioni utilizzate, ma mostra anche quelle che potrebbero essere sottoutilizzate.
...
Il numero di tipi di pubblico mappati mostrato nella colonna [!UICONTROL Audience count] è accurato all&#39;ultima istantanea giornaliera. La mappatura di un nuovo pubblico sulla destinazione non aggiorna il conteggio fino allo scatto dell’istantanea successiva.

Selezionate il nome di una destinazione dall&#39;elenco visualizzato sul widget per passare ai dettagli della destinazione per quella particolare destinazione. È inoltre possibile selezionare **[!UICONTROL View All]** per passare alla scheda **[!UICONTROL Browse]** e quindi selezionare il nome di una destinazione per visualizzarne i dettagli.
...
Il widget **[!UICONTROL Recently created destinations]** ti consente di visualizzare un elenco delle destinazioni configurate più di recente dalla tua organizzazione.
...
Se si seleziona il nome di una destinazione dall&#39;elenco visualizzato nel widget, verranno visualizzati i dettagli della destinazione collegati dalla scheda **[!UICONTROL Browse]**. È inoltre possibile selezionare **[!UICONTROL View All]** per passare alla scheda **[!UICONTROL Browse]** e quindi selezionare il nome di una destinazione per visualizzarne i dettagli.
...
Il widget **[!UICONTROL Recently activated audiences]** fornisce un elenco degli ultimi tipi di pubblico mappati a una destinazione. Questo elenco fornisce uno snapshot dei tipi di pubblico e delle destinazioni attivamente utilizzati nel sistema e può essere utile per risolvere eventuali mappature errate.

La data [!UICONTROL Updated] visualizzata mostra l&#39;ultima volta che il pubblico è stato attivato nella destinazione ed è accurata rispetto all&#39;ultima istantanea giornaliera. In altre parole, se attivi un pubblico nella destinazione, la data aggiornata non cambierà fino a quando non viene acquisita l’istantanea successiva.

Selezionando il nome di un pubblico dall’elenco visualizzato sul widget, puoi passare ai dettagli del pubblico. È inoltre possibile selezionare **[!UICONTROL View All]** per passare alla scheda [!UICONTROL Audiences] [!UICONTROL Browse] e quindi selezionare il nome di un pubblico per visualizzarne i dettagli.
...
Il widget **[!UICONTROL Recently activated audiences by destination]** visualizza i primi cinque tipi di pubblico attivati più di recente in ordine decrescente in base alla destinazione scelta nel menu a discesa Panoramica. È simile al widget [!UICONTROL Recently activated audiences], ma i dati visualizzati **solo** si applicano alla destinazione selezionata.
...
Il widget **[!UICONTROL Audience size trend]** fornisce un pulsante [!UICONTROL Captions] in alto a destra del widget. Selezionare **[!UICONTROL Captions]** per aprire la finestra di dialogo dei sottotitoli automatici. Un modello di apprendimento automatico genera automaticamente sottotitoli per descrivere le tendenze chiave e gli eventi importanti analizzando i dati del grafico e del pubblico.
...
Il widget **[!UICONTROL Unmapped audiences by identity]** elenca i primi cinque tipi di pubblico **non mappati** classificati in base al conteggio di identità decrescente per una determinata destinazione e identità. Evidenzia i tipi di pubblico più utili da mappare sull’account di destinazione scelto in base all’ID scelto.
...
Il widget **[!UICONTROL Common audiences]** fornisce un elenco dei primi cinque tipi di pubblico attivati nell&#39;account di destinazione scelto nella parte superiore della pagina e la destinazione selezionata nel menu a discesa del widget. L’elenco dei tipi di pubblico viene ordinato in base a quanto recentemente sono stati attivati. Il pubblico attivato più recentemente viene visualizzato in alto.
...
Il widget [!UICONTROL Mapped audiences] visualizza il numero totale di tipi di pubblico mappati che possono essere attivati nella destinazione selezionata nella parte superiore della pagina.

Seleziona **[!UICONTROL Audiences]** per passare alla scheda Dashboard tipi di pubblico [!UICONTROL Browse]. In questa area di lavoro viene visualizzato un elenco di tutte le definizioni di segmenti per l’organizzazione.
...
Tutte le dimensioni del pubblico nel sistema sono ordinate da un pubblico di dimensioni alte a uno di dimensioni basse, come indicato nella colonna [!UICONTROL LATEST SIZE].
...
Se un pubblico all&#39;interno del widget [!UICONTROL Mapped audience health] si sta deviando di un ampio margine, è necessario fare riferimento al grafico di tendenza delle dimensioni del pubblico e individuare il pubblico anomalo. Questa tendenza può fornire ulteriore insight allo stato di salute del pubblico.
...
Il widget [!UICONTROL Destinations count] fornisce il numero totale di endpoint disponibili in cui un pubblico può essere attivato e distribuito all&#39;interno del sistema. Questo numero include sia le destinazioni attive che quelle inattive.

Al di sotto del conteggio totale, selezionare **[!UICONTROL Destinations]** per passare alla scheda Sfoglia destinazioni. In questa pagina sono elencate tutte le destinazioni con cui è stata stabilita una connessione fino a oggi.

![Widget conteggio destinazioni.](../images/destinations/destinations-count.png)

### [!UICONTROL Destination status] {#destination-status}

Il widget [!UICONTROL Destination status] visualizza il numero totale di destinazioni abilitate come una singola metrica e utilizza un grafico ad anello per illustrare la differenza proporzionale tra le destinazioni abilitate e disabilitate.
...
Il widget [!UICONTROL Activated audiences across all destinations] fornisce il numero totale di tipi di pubblico attivati su tutte le destinazioni in una singola metrica. Questo numero corrisponde all&#39;istantanea più recente.
...
Selezionare **[!UICONTROL Audiences]** per passare alla scheda delle destinazioni [!UICONTROL Browse]. Questa pagina fornisce un elenco di tutte le destinazioni abilitate e una serie di metriche rilevanti. Per ulteriori informazioni sulla scheda [[!UICONTROL Browse]](../../destinations/ui/destinations-workspace.md#browse), vedere la documentazione.
...
Selezionare **[!UICONTROL Audiences]** per passare alla pagina dei dettagli del dashboard delle destinazioni. Nella scheda [!UICONTROL Activation data] viene visualizzato un elenco dei tipi di pubblico mappati sulla destinazione, inclusa la data di inizio e di fine (se applicabile) e altre informazioni rilevanti per l&#39;esportazione dei dati, come tipo di esportazione, pianificazione e frequenza. Per visualizzare i dettagli su un determinato pubblico, selezionarne il nome dalla colonna [!UICONTROL Audience Name].
