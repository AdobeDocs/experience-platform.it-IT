...
Mediante la valutazione pianificata, l’organizzazione IMS può creare una pianificazione ricorrente per eseguire automaticamente i processi di esportazione.
...
| `status` | Lo stato di partecipazione al segmento come parte della richiesta corrente. Deve essere uguale a uno dei seguenti valori noti: <ul><li>`existing`: l’entità continua a essere nel segmento.</li><li>`realized`: l’entità sta entrando nel segmento.</li><li>`exited`: uscita dell’entità dal segmento.</li></ul> |
