...
title: Creare un rapporto sulle tendenze degli eventi
description: Scopri come scrivere query che utilizzano Experience Events per creare un rapporto con tendenze di eventi su un intervallo di date specificato, raggruppati per data.
...
Nell&#39;esempio seguente viene creato un report con tendenze di eventi su un intervallo di date specificato, raggruppati per data. In particolare, questo esempio SQL riassume diversi valori di analisi come `A`, `B`e `C`, quindi riassume il numero di volte in cui i parchi sono stati visualizzati nel corso di un mese.

La colonna timestamp si trova in [!DNL Experience Event] i set di dati sono in formato UTC. Nell&#39;esempio viene utilizzato il `from_utc_timestamp()` per trasformare la marca temporale da UTC a EDT e quindi utilizza la funzione `date_format()` per isolare la data dal resto della marca temporale.
...
Leggendo questo documento, hai una migliore comprensione di come utilizzare Query Service con [!DNL Experience Events] per monitorare efficacemente le tendenze comportamentali tra gli utenti.

Per informazioni su altri casi d’uso basati sui visitatori che utilizzano [!DNL Experience Events], leggi i seguenti documenti:
