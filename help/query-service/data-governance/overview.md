...
#### Creare restrizioni di accesso basate su campi per i set di dati accelerati {#create-field-based-access-restrictions-on-accelerated-datasets}

Con la [capacità di controllo dell&#39;accesso basata sugli attributi](../../access-control/abac/overview.md) puoi definire ambiti di utilizzo organizzativi o dati sui set di dati di fatto e dimensione in [memoria accelerata](../data-distiller/query-accelerated-store/send-accelerated-queries.md). Questo consente agli amministratori di gestire l’accesso a segmenti specifici e di gestire meglio l’accesso dato a utenti o gruppi di utenti.

Per creare restrizioni di accesso basate su campi sui set di dati accelerati, puoi utilizzare le query CTAS del servizio query per creare set di dati accelerati e strutturare questi set di dati in base agli schemi XDM esistenti o agli schemi ad hoc. Gli amministratori possono quindi [aggiungere e modificare le etichette di utilizzo dei dati per lo schema](../../xdm/tutorials/labels.md#edit-the-labels-for-the-schema-or-field) o [schema ad hoc](./ad-hoc-schema-labels.md#edit-governance-labels). Puoi applicare, creare e modificare le etichette agli schemi da [!UICONTROL Etichette] nell&#39;area di lavoro [!UICONTROL Schemi] Interfaccia utente.

Le etichette di utilizzo dei dati possono anche essere [applicato o modificato direttamente nel set di dati](../../data-governance/labels/user-guide.md#add-labels) tramite l&#39;interfaccia utente Datasets o creato da Access Control [!UICONTROL Etichette] workspace. Consulta la guida su come [creare una nuova etichetta](../../access-control/abac/ui/labels.md) per ulteriori informazioni.

L’accesso dell’utente a singole colonne può quindi essere controllato dalle etichette di utilizzo dei dati allegate e dai set di autorizzazioni applicati ai ruoli assegnati agli utenti.
