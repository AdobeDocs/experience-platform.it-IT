...
Le query che utilizzano blocchi anonimi per comporre le relative istruzioni SQL sono separate nelle singole sottoquery. Questo ti consente di controllare i dettagli di esecuzione per ogni blocco di query singolarmente.

>[!NOTE]
>
>I dettagli di esecuzione di un blocco anonimo che utilizza il comando DROP **not** essere segnalati come sottoquery separata. Sono disponibili dettagli di esecuzione separati per le query CTAS, le query ITAS e le istruzioni COPY utilizzate come sottoquery a blocchi anonimi. I dettagli di esecuzione del comando DROP non sono attualmente supportati.
...
Le sottoquery di blocchi anonimi hanno schede a sinistra dello stato di esecuzione. Seleziona una scheda per visualizzare i dettagli dell’esecuzione.
