...
Nell&#39;interfaccia utente di [!DNL Experience Platform], selezionare **[!UICONTROL Queries]** nel menu di navigazione a sinistra per aprire l&#39;area di lavoro di Query Service. Quindi, per iniziare a scrivere le query, seleziona **[!UICONTROL Create Query]** in alto a destra dello schermo. Questo collegamento è disponibile da una qualsiasi delle pagine dell’area di lavoro Servizio query.
...
>[!TIP]
>
>È possibile [!UICONTROL Disable syntax auto complete] durante l&#39;authoring di una query senza perdere l&#39;avanzamento.
...
Icona [!UICONTROL Run selected query]. Questa icona è disattivata per impostazione predefinita finché non selezioni la sintassi della query all’interno dell’editor.

![Editor query con l&#39;icona [!UICONTROL Run selected query] evidenziata.](../images/ui/query-editor/run-selected-query.png)

### Annulla sessione editor query {#cancel-query}

Controlla l’esecuzione delle query e migliora la produttività annullando le query con tempi di esecuzione lunghi. Questa azione cancella l&#39;Editor query durante l&#39;esecuzione di una query. Tieni presente che la query continua a essere eseguita in background. Se si tratta di una query CTAS, verrà comunque generato un set di dati di output. Per annullare l&#39;esecuzione nell&#39;editor e continuare a comporre un&#39;istruzione SQL, selezionare **[!UICONTROL Cancel query]** dopo aver eseguito una query.

![Editor query con [!UICONTROL Cancel query] evidenziato.](../images/ui/query-editor/cancel-query-run.png)

Viene visualizzata una finestra di dialogo di conferma. Selezionare **[!UICONTROL Confirm]** per annullare l&#39;esecuzione della query.
...
L’editor delle query può contenere un massimo di 50.000 righe. Puoi scegliere il numero di righe da visualizzare contemporaneamente nella console dell’editor delle query. Per modificare il numero di righe visualizzate nella console, seleziona il menu a discesa **[!UICONTROL Result count]** e scegli tra le opzioni 50, 100, 150, 300, 500 e 1000.
...
[!UICONTROL Query Editor] è organizzato in modo da semplificare al massimo la scrittura delle query. La schermata seguente mostra come viene visualizzato l&#39;editor nell&#39;interfaccia utente, con il campo di immissione SQL e **Riproduci** evidenziati.
...
La funzionalità [!UICONTROL Format text] rende la query più leggibile aggiungendo uno stile di sintassi standardizzato. Selezionare **[!UICONTROL Format text]** per standardizzare tutto il testo nell&#39;editor di query.

>[!NOTE]
>
>La funzionalità [!UICONTROL Format text] non funziona con blocchi anonimi. Per informazioni su come concatenare una o più istruzioni SQL in sequenza, vedere la [documentazione sui blocchi anonimi](../key-concepts/anonymous-block.md).

![Editor query con [!UICONTROL Format text] e istruzioni SQL evidenziate.](../images/ui/query-editor/format-text.png)
...
Per visualizzare una query nell&#39;editor query, selezionare qualsiasi modello salvato dalla scheda [!UICONTROL Templates]. Il pannello Dettagli query fornisce ulteriori informazioni e strumenti per gestire la query selezionata. Mostra inoltre metadati utili, ad esempio l’ultima volta che la query è stata modificata e chi l’ha modificata, se applicabile.

>[!NOTE]
>
>Le opzioni [!UICONTROL View schedule], [!UICONTROL Add schedule] e [!UICONTROL Delete query] sono disponibili solo dopo il salvataggio della query come modello. L&#39;opzione [!UICONTROL Add schedule] consente di accedere direttamente al generatore di pianificazioni dall&#39;editor di query. L&#39;opzione [!UICONTROL View schedule] consente di accedere direttamente all&#39;inventario di pianificazione per la query. Consulta la documentazione sulle pianificazioni delle query per scoprire come [creare pianificazioni delle query nell&#39;interfaccia utente](./query-schedules.md#create-schedule).
...
Per generare un set di dati, selezionare **[!UICONTROL Run as CTAS]**. Viene visualizzata la finestra di dialogo **[!UICONTROL Enter output dataset details]**. Immettere un nome e una descrizione, quindi selezionare **[!UICONTROL Run as CTAS]**. Il nuovo set di dati viene visualizzato nella scheda Sfoglia di **[!UICONTROL Datasets]**. Per ulteriori informazioni sui set di dati disponibili per la tua organizzazione, consulta [la documentazione sulla visualizzazione dei set di dati](../../catalog/datasets/user-guide.md#view-datasets).

>[!NOTE]
>
>L&#39;opzione [!UICONTROL Run as CTAS] è disponibile solo se per la query è stato pianificato **not**.

![Finestra di dialogo [!UICONTROL Enter output dataset details].](../images/ui/query-editor/output-dataset-details.png)

Dopo aver eseguito l&#39;azione **[!UICONTROL Run as CTAS]**, viene visualizzato un messaggio di conferma per segnalare l&#39;azione riuscita. Questo messaggio a comparsa contiene un collegamento che consente di accedere facilmente all’area di lavoro dei registri delle query. Per ulteriori informazioni sui registri query, consulta la [documentazione dei registri query](./query-logs.md).

### Salvataggio delle query {#saving-queries}

L’editor delle query fornisce una funzione di salvataggio che consente di salvare una query e lavorarci in un secondo momento. Per salvare una query, selezionare **[!UICONTROL Save]** nell&#39;angolo superiore destro di Query Editor. Prima di salvare una query, è necessario specificare un nome per la query utilizzando il pannello **[!UICONTROL Query Details]**.

>[!NOTE]
>
>Le query denominate e salvate in utilizzando l&#39;Editor query sono disponibili come modelli nella scheda Dashboard query [!UICONTROL Templates]. Per ulteriori informazioni, consulta la [documentazione dei modelli](./query-templates.md).
...
>[!NOTE]
>
>Le query pianificate che non vanno a buon fine per dieci esecuzioni consecutive vengono automaticamente impostate sullo stato [!UICONTROL Quarantined]. Una query con questo stato richiede l’intervento dell’utente prima di poter eseguire ulteriori esecuzioni. Per ulteriori dettagli, consulta la documentazione sulle [query in quarantena](./monitor-queries.md#quarantined-queries).
...
Tutte le query pianificate vengono aggiunte all&#39;elenco nella scheda [!UICONTROL Scheduled queries]. Da tale area di lavoro è possibile monitorare lo stato di tutti i processi di query pianificati tramite l’interfaccia utente. Nella scheda [!UICONTROL Scheduled queries], puoi trovare informazioni importanti sull&#39;esecuzione della query e abbonarti agli avvisi. Le informazioni disponibili includono lo stato, i dettagli della pianificazione e i messaggi/codici di errore in caso di esecuzione non riuscita. Per ulteriori informazioni, vedere il documento [Monitorare le query pianificate](./monitor-queries.md).


### Come trovare le query precedenti {#previous-queries}

Tutte le query eseguite dall&#39;editor di query vengono acquisite nella tabella Registro. È possibile utilizzare la funzionalità di ricerca nella scheda **[!UICONTROL Log]** per trovare le esecuzioni delle query. Le query salvate sono elencate nella scheda **[!UICONTROL Templates]**.

Se è stata pianificata una query, la scheda [!UICONTROL Scheduled Queries] fornisce una migliore visibilità tramite l&#39;interfaccia utente per tali processi di query. Per ulteriori informazioni, vedere la [documentazione sul monitoraggio delle query](./monitor-queries.md).
...
Per spostarsi nel database con il browser oggetti, immettere un nome di tabella nel campo di ricerca oppure selezionare **[!UICONTROL Tables]** per espandere l&#39;elenco dei set di dati e delle tabelle disponibili. Quando si utilizza il campo di ricerca, l’elenco delle tabelle disponibili viene filtrato in modo dinamico in base all’input.
...
Per eseguire una query nell&#39;editor delle query, è possibile immettere SQL nell&#39;editor o caricare una query precedente dalla scheda **[!UICONTROL Log]** o **[!UICONTROL Templates]** e selezionare **Esegui**. Lo stato dell&#39;esecuzione della query viene visualizzato nella scheda **[!UICONTROL Console]** di seguito, mentre i dati di output vengono visualizzati nella scheda **[!UICONTROL Results]**.
...
Per scaricare i risultati della query, selezionare **[!UICONTROL Download]** nell&#39;angolo superiore destro della scheda Editor query **[!UICONTROL Result]**. Scegliere quindi **[!UICONTROL CSV]**, **[!UICONTROL XLSX]** o **[!UICONTROL JSON]** dal menu a discesa. Il file viene scaricato automaticamente nel computer locale. Scegli il formato adatto al tuo caso d’uso: CSV per esportazioni leggere, XLSX per fogli di calcolo formattati o JSON per la gestione di dati strutturati.

>[!NOTE]
>
>Se manca il pulsante **[!UICONTROL Download]**, controlla i risultati della query. Il pulsante viene visualizzato solo quando vengono restituiti record. Se non viene restituito alcun record, nella scheda **[!UICONTROL Result]** viene visualizzato il messaggio &#39;Nessun risultato&#39; e l&#39;opzione di download è disabilitata.
...
Dopo aver eseguito una query corretta, selezionare **[!UICONTROL View results]** nella scheda **[!UICONTROL Result]** per aprire una visualizzazione a schermo intero tabulata dei risultati.
...
>[!VIDEO](https://video.tv.adobe.com/v/39842?captions=ita&quality=12&learn=on)

## Monitorare e gestire le sessioni simultanee {#monitor-manage-sessions}

Utilizza la gestione delle sessioni per visualizzare le sessioni attive dell’editor delle query nelle sandbox, identificare le sessioni inattive e terminarle per liberare capacità condivisa. Non è possibile interrompere le sessioni che eseguono query in modo attivo. Questa funzionalità è destinata solo agli amministratori e richiede l&#39;autorizzazione **[!UICONTROL Manage Query Session]**.

Per accedere alla gestione delle sessioni, selezionare la scheda **[!UICONTROL Admin]** nell&#39;area di lavoro Servizio query. Per istruzioni dettagliate su come visualizzare i dettagli della sessione, interpretare lo stato della sessione e terminare le sessioni inattive, vedere [Gestire le sessioni di Query Service](session-management.md).
