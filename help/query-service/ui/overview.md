...
Adobe Experience Platform Query Service fornisce un’interfaccia utente che può essere utilizzata per scrivere ed eseguire query, visualizzare le query eseguite in precedenza e accedere a quelle salvate dagli utenti della tua organizzazione. Per accedere all&#39;interfaccia utente in [Adobe Experience Platform](https://platform.adobe.com), seleziona **[!UICONTROL Queries]** nell&#39;area di navigazione a sinistra. Verrà visualizzato [!UICONTROL Queries] [!UICONTROL Overview].

![Area di lavoro di Query Service con query e scheda Panoramica evidenziate.](../images/ui/overview/queries-overview.png)

## Panoramica {#overview}

La scheda [!UICONTROL Overview] fornisce un punto di ingresso semplificato per l&#39;utilizzo di query e modelli di Data Distiller. Qui puoi accedere a tutte le funzioni necessarie per scrivere query, esplorare set di dati e analizzare i dati sul pubblico, per garantire un flusso di lavoro fluido per le analisi dei dati e gli approfondimenti sul pubblico. Utilizza questa panoramica per scoprire cosa puoi ottenere con Data Distiller e le metriche chiave sull’utilizzo di Query Service.

### Pannelli principali {#main-panels}

La pagina [!UICONTROL Overview] contiene diverse sezioni principali utili per iniziare:

1. Selezionare **[!UICONTROL Create query]** per passare rapidamente all&#39;editor query per scrivere ed eseguire nuove query.
2. Selezionare **[!UICONTROL Learn more]** per visualizzare la documentazione dettagliata su come **[!UICONTROL Write queries]**.
3. Selezionare **[!UICONTROL Get started]** nella sezione **[!UICONTROL Discover Data Distiller]** per aprire la panoramica di Data Distiller e scoprire le funzionalità disponibili.

![Area di lavoro di Query Service con Crea query, Ulteriori informazioni ed Evidenzia Introduzione.](../images/ui/overview/main-panels.png)

### Funzionalità di Data Distiller {#data-distiller-capabilities}

La sezione [!UICONTROL Data Distiller capabilities] fornisce collegamenti alla documentazione per le funzioni più avanzate di Data Distiller:

- **[[!UICONTROL Data exploration]](../use-cases/data-exploration.md)**: scopri come esplorare, risolvere i problemi e verificare i dati acquisiti in batch utilizzando SQL.
- **[[!UICONTROL Derived datasets for Experience Platform applications]](../data-distiller/derived-datasets/overview.md)**: scopri come creare set di dati derivati per supportare casi d&#39;uso complessi e diversi che ottimizzano l&#39;utilità dati.
- **[[!UICONTROL AI/ML pipelines]](../data-distiller/ml-feature-pipelines/overview.md)**: scopri i concetti importanti alla base degli strumenti di apprendimento automatico preferiti e come creare modelli personalizzati che supportano i casi di utilizzo di marketing. Questa serie di guide descrive i passaggi necessari per creare pipeline di funzioni che preparino i dati da Experience Platform per alimentare modelli personalizzati nell’ambiente di apprendimento automatico.
- **[[!UICONTROL SQL insights]](../data-distiller/sql-insights/overview.md)**: scopri le funzionalità chiave e i passaggi necessari per sviluppare un dashboard di approfondimenti da SQL con Data Distiller.
...
- **[[!UICONTROL Decile-based derived datasets]](../use-cases/deciles-use-case.md)**: scopri come creare set di dati derivati basati su decile per la segmentazione e la creazione di pubblico in Adobe Experience Platform. Utilizzando uno scenario di fidelizzazione della compagnia aerea, vengono trattati la progettazione dello schema, i calcoli decile e gli esempi di query per la classificazione e l’aggregazione dei dati.
- **[[!UICONTROL Customer lifetime value]](../use-cases/customer-lifetime-value.md)**: scopri come tenere traccia e visualizzare il valore del ciclo di vita del cliente con Real-Time CDP e dashboard personalizzati. Utilizza queste informazioni per sviluppare strategie per acquisire nuovi clienti, mantenere quelli esistenti e massimizzare i margini di profitto.
- **[[!UICONTROL Propensity score]](../use-cases/propensity-score.md)**: scopri come determinare i punteggi di tendenza utilizzando modelli predittivi di apprendimento automatico. Questa guida tratta l’invio di dati per la formazione, l’applicazione di modelli addestrati con SQL e la previsione della probabilità di acquisto dei clienti.
- **[[!UICONTROL Consent analysis]](../../dashboards/insights-use-cases/consent-analysis.md)**: scopri come analizzare e tenere traccia del consenso dei clienti utilizzando Real-Time CDP, Query Service e Data Distiller. Questa guida descrive come creare dashboard di consenso, perfezionare la segmentazione, monitorare le tendenze e garantire la conformità, aiutandoti a creare fiducia e a fornire esperienze personalizzate.
- **[[!UICONTROL Fuzzy match]](../use-cases/fuzzy-match.md)**: scopri come eseguire una corrispondenza &quot;fuzzy&quot; sui dati di Experience Platform per trovare corrispondenze approssimative e analizzare la somiglianza delle stringhe tra i set di dati. Segui questa guida per risparmiare tempo e rendere i tuoi dati più accessibili. L’esempio illustra come abbinare gli attributi della camera di albergo tra due set di dati dell’agenzia di viaggi, mostrando come abbinare, confrontare e riconciliare in modo efficiente set di dati di grandi dimensioni e complessi per coerenza e precisione.

![Area di lavoro di Query Service con la sezione degli esempi di Data Distiller evidenziata.](../images/ui/overview/data-distiller-examples.png)

### Metrica chiave {#key-metrics}

La sezione Metriche chiave presenta visualizzazioni di dati importanti che consentono di monitorare l’utilizzo di Query Service. Per ogni grafico è possibile selezionare i puntini di sospensione (`...`) in alto a destra seguiti da [!UICONTROL View more] per visualizzare una tabella dei risultati oppure scaricare i dati come file CSV da visualizzare in un foglio di calcolo. Per ulteriori dettagli, fare riferimento alla [Guida per ulteriori informazioni](../../dashboards/sql-insights-query-pro-mode/view-more.md).
...
Nella scheda [!UICONTROL Result] viene visualizzato l&#39;output tabulare della query dopo l&#39;esecuzione. Utilizza questa scheda per rivedere i risultati, convalidare l’output e intraprendere azioni di follow-up direttamente nell’interfaccia. Da questa vista puoi effettuare le seguenti operazioni:
...
È possibile definire i parametri durante la scrittura delle query, quindi assegnare i valori tramite la scheda [!UICONTROL Query parameters] prima di eseguirli. Le query con parametri sono particolarmente utili per le query pianificate o per i modelli di query condivisi nell’organizzazione.
...
Una volta pianificata, la query viene visualizzata nell&#39;elenco delle query pianificate nella scheda [!UICONTROL Scheduled Queries]. Per informazioni dettagliate sulla query, sulle esecuzioni, sul creatore e sugli intervalli, seleziona una query pianificata dall’elenco.
...
| Colonna | Descrizione |
| --- | --- |
| **[!UICONTROL Name]** | Il campo del nome corrisponde al nome del modello o ai primi caratteri della query SQL. Tutte le query create tramite l’interfaccia utente con l’editor delle query sono denominate all’inizio. Se la query è stata creata tramite l’API, il nome della query è un frammento dell’istruzione SQL iniziale utilizzata per creare la query. |
| **[!UICONTROL Template]** | Nome del modello della query. Selezionate un nome di modello per passare all&#39;editor di query. Il modello di query viene visualizzato nell’editor delle query per comodità. Se non è presente alcun nome di modello, la riga viene contrassegnata con un trattino e non è possibile reindirizzare all’editor delle query per visualizzare la query. |
| **[!UICONTROL SQL]** | Frammento della query SQL. |
| **[!UICONTROL Run frequency]** | Questa colonna indica la frequenza con cui è impostata l&#39;esecuzione della query. I valori disponibili sono `Run once` e `Scheduled`. Le query possono essere filtrate in base alla loro frequenza di esecuzione. |
| **[!UICONTROL Created by]** | Nome dell&#39;utente che ha creato la query. |
| **[!UICONTROL Created]** | La marca temporale in formato UTC in cui è stata creata la query. |
| **[!UICONTROL Last run timestamp]** | Il timestamp più recente in cui è stata eseguita la query. In questa colonna viene evidenziato se una query è stata eseguita in base alla pianificazione corrente. |
| **[!UICONTROL Last run status]** | Stato dell’esecuzione della query più recente. I tre valori di stato sono: `successful` `failed` o `in progress`. |
...
La scheda **[!UICONTROL Templates]** mostra le query salvate dagli utenti dell&#39;organizzazione. È utile considerarli come progetti di query, in quanto le query salvate qui potrebbero essere ancora in fase di costruzione. Le query visualizzate nella scheda **[!UICONTROL Templates]** vengono visualizzate anche come query di esecuzione nella scheda **[!UICONTROL Log]** se sono state precedentemente eseguite da Query Service.
...
| **[!UICONTROL Modified by]** | Ultimo utente che ha modificato la query. Qualsiasi utente dell’organizzazione con accesso a Query Service può modificare le query. |
| **[!UICONTROL Last modified]** | La data e l’ora dell’ultima modifica alla query, nel fuso orario del browser. |
...
La scheda **[!UICONTROL Log]** fornisce un elenco delle query eseguite in precedenza. Per impostazione predefinita, il registro elenca le query in ordine cronologico inverso.
...
| Colonna | Descrizione |
| --- | --- |
| **[!UICONTROL Name]** | Il nome della query, costituito dai primi caratteri della query SQL. Selezionare il nome del modello per aprire la visualizzazione [!UICONTROL Query log details] per l&#39;esecuzione. È possibile utilizzare la barra di ricerca per eseguire una ricerca in base al nome di una query. Le ricerche distinguono tra maiuscole e minuscole. |
| **[!UICONTROL Start time]** | L’ora in cui è stata eseguita la query. |
| **[!UICONTROL Complete time]** | L’ora in cui è stata completata la query. |
| **[!UICONTROL Status]** | Stato corrente della query. |
| **[!UICONTROL Dataset]** | Set di dati di input utilizzato dalla query. Seleziona il set di dati per passare alla schermata dei dettagli del set di dati di input. |
...
Nella scheda **[!UICONTROL Credentials]** vengono visualizzate sia le credenziali in scadenza che quelle senza scadenza. Per ulteriori informazioni su come utilizzare queste credenziali per connettersi con client esterni, leggere la [guida delle credenziali](../clients/overview.md).

![Dashboard delle query con la scheda Credenziali evidenziata.](../images/ui/overview/credentials.png)

## Amministrazione {#admin}

Utilizzare la scheda **[!UICONTROL Admin]** per monitorare e gestire le sessioni simultanee di Query Editor nell&#39;organizzazione. Questa funzionalità è destinata agli amministratori e non è necessaria per la scrittura o l’esecuzione di query.

Dalla scheda **[!UICONTROL Admin]**, gli amministratori possono visualizzare le sessioni attive tra sandbox e terminare le sessioni inattive per liberare la capacità condivisa. Questa azione non interrompe le query in esecuzione attiva. Per istruzioni dettagliate e requisiti delle autorizzazioni, consulta la [guida alla gestione delle sessioni di Query Service](session-management.md).
