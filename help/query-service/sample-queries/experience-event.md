...
title: Query di esempio per eventi di esperienza
type: Tutorial
...
Oltre alle query SQL standard, Adobe Experience Platform [!DNL Query Service] supporta la scrittura di query utilizzando [!DNL Experience Events]. Gli eventi di esperienza sono rappresentati dalla classe Experience Data Model (XDM) ExperienceEvent , che acquisisce uno snapshot immutabile e non aggregato del sistema quando un utente interagisce con un sito web o un servizio, e possono quindi essere utilizzati per l’analisi del dominio del tempo.

Ulteriori informazioni su XDM e [!DNL Experience Events] si trova nella [[!DNL XDM System] panoramica](../../xdm/home.md). Combinando [!DNL Query Service] con [!DNL Experience Events], è possibile monitorare efficacemente le tendenze comportamentali tra gli utenti. Il seguente documento fornisce esempi di query che coinvolgono [!DNL Experience Events].

## Creazione di un rapporto con tendenze di eventi per giorno su un intervallo di date specifico

Nell&#39;esempio seguente viene creato un report con tendenze di eventi su un intervallo di date specificato, raggruppati per data. In particolare, riassume vari valori di analisi come A, B e C, quindi riassume il numero di volte in cui i parchi sono stati visualizzati.

La colonna timestamp si trova in [!DNL Experience Event] i set di dati sono in UTC. Nell&#39;esempio seguente viene utilizzato il `from_utc_timestamp()` per trasformare la marca temporale da UTC a EDT. Quindi utilizza la `date_format()` per isolare la data dal resto della marca temporale.
...
Per ulteriori informazioni sulle query di esempio che utilizzano le funzioni definite in Adobe (ADF, Defined Functions), consulta la guida Funzioni definite in Adobe . Per informazioni generali sull’esecuzione delle query, leggere il [guida sull’esecuzione delle query in Query Service](../best-practices/writing-queries.md).
