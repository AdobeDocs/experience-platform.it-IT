---
title: Recuperare il contenuto di personalizzazione da altre soluzioni Adobe
...
# Recuperare il contenuto di personalizzazione dalle soluzioni Adobe
...
* **Ambito**: Il campo di applicazione della decisione.
   ...
   * Per [!DNL Offer Decisioning], sono le stringhe codificate Base64 di JSON contenenti gli ID di attività e posizionamento che desideri che il servizio offer decisioning utilizzi per proporre offerte.
...
| --- | --- | --- |
| `schemas` | `String[]` | *Facoltativo*. Elenco degli schemi utilizzati nella decisione, per selezionare il tipo di offerte restituite. |
...
| `payload.scopeDetails.decisionProvider` | Stringa | Questo sarà `TGT` se si utilizza Adobe Target o `ODE` se si utilizza [!DNL Offer Decisioning]. |
...
## Recuperare il contenuto {#retrieve-content}
