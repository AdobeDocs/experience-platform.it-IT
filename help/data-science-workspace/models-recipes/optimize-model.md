---
keywords: Experience Platform;ottimizzare;modello;Data Science Workspace;argomenti popolari;informazioni sul modello;;optimize;model;Data Science;popular topic;model insights
...
Esistono due modi per inserire le metriche nelle ricette. Una consiste nell’utilizzare le metriche di valutazione predefinite fornite da SDK, l’altra consiste nel scrivere metriche di valutazione personalizzate.
...
Lo scopo del metodo `split()` è quello di immettere dati e generare un set di dati di formazione e di test. Nel nostro esempio, il metodo `split()` inserisce i dati utilizzando il SDK `DataSetReader`, quindi pulisce i dati rimuovendo colonne non correlate. A questo punto, vengono create feature aggiuntive a partire dalle feature raw esistenti nei dati.
