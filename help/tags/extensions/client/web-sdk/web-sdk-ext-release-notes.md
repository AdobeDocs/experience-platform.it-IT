...
- Contiene [versione 2.33.0](/help/collection/js/release-notes.md) di Adobe Experience Platform Web SDK.
- È stato rimosso il flag beta dalle sezioni Advertising e Push notifications nella pagina di configurazione dell’estensione.
- È stato risolto un problema che causava la visualizzazione di errori nella configurazione delle notifiche push prima della modifica.
- È stato risolto un problema a causa del quale gli eventi di analisi venivano sovrascritti nelle azioni sequenziali Aggiorna variabile.
...
- Contiene [versione 2.31.1](/help/collection/js/release-notes.md) di Adobe Experience Platform Web SDK.
- È stato risolto un problema che impediva la modifica di [impostazioni di configurazione di Adobe Advertising](configure/advertising.md) nelle azioni [Invia evento](actions/send-event.md).
- È stato aggiunto un fallback a `edge.adobedc.net` per l&#39;impostazione di configurazione del [dominio Edge](configure/general.md) quando `tenant-id` non è disponibile.
