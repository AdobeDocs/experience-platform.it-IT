...
|---|---|---|
| 1.0.52 | 27 aprile 2026 | <ul><li>È stato aggiunto il tracciamento per le risorse in cui CSS carica le immagini in background di un elemento nel DOM.</li><li>È stato aggiunto un array `permanentlyBlockedURLs` hardcoded contenente [map.googleapis.com](https://maps.googleapis.com) e [mapsresources-pa.googleapis.com](https://mapsresources-pa.googleapis.com). Questi URL vengono sempre bloccati per impostazione predefinita nella libreria Content Analytics.</li><li>`idSource` e `channel` campi aggiunti alle richieste di raccolta dati XDM.</li></ul> |
| 1.0.51 | 13 marzo 2026 | <ul><li>È stato corretto un bug non grave che poteva causare la memorizzazione di `experienceIDs` nella cache durante la navigazione tra le pagine.</li><li>È stato risolto un problema relativo all’acquisizione dei parametri querystring dell’esperienza. I parametri di query funzionano come segue:<ul><li>Il campo dei parametri di query è vuoto: nell’ID esperienza non vengono acquisiti parametri querystring.</li><li>I parametri di query sono definiti in modo esplicito (ad esempio, uno, due, tre): nell’ID esperienza vengono acquisiti solo i parametri e i valori querystring.</li><li>Il parametro Query è impostato su un carattere jolly (`.*`): l&#39;intera stringa document.location.search è inclusa nell&#39;URL.</li></ul></li></ul> |
| 1.0.49 | 12 settembre 2025 | <ul><li>È stato corretto un bug non grave a causa del quale l’interfaccia utente dell’estensione Tag non veniva caricata affatto se l’utente non disponeva di autorizzazioni per lo stream di dati. L&#39;interfaccia utente visualizzerà ora un avviso di autorizzazione nell&#39;opzione dello stream di dati **[!UICONTROL Choose from list]** e consentirà comunque all&#39;utente di immettere manualmente i valori.</li><li>È stato aggiornato un problema di percorso per l10n.</li><li>È stato risolto un problema a causa del quale alcune immagini, elementi secondari di elementi principali non immagine, non acquisivano correttamente i clic delle risorse per tali elementi immagine secondari.</li><li>Se un utente ha configurato WebSDK in tag con un nome di istanza diverso da `alloy`, la libreria Content Analytics rileverà la prima istanza della libreria WebSDK e la utilizzerà per inviare eventi Content Analytics.</li></ul> |
