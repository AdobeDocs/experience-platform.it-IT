...
* Aggiunge nuovi eventi di delegato nativi per il Web:
   * Keydown
   * KeyUp
* Aggiunge la possibilità di eseguire il test rispetto a molti valori ( opzioni &quot;Aggiungi un altro&quot;) per i seguenti delegati:
   * Eventi
      * Cambia
   * Condizioni
      * Cookie
      ...
      * Traffic Source
      * Variable
* Modifica il delegato events/EntersViewport per utilizzare il [API per l’osservatore dell’intersezione](https://developer.mozilla.org/en-US/docs/Web/API/Intersection_Observer_API) invece del rilevamento manuale degli elementi che entrano nel riquadro di visualizzazione.
* Rimuove il codice che eseguiva la migrazione dei cookie DTM a LocalStorage.
