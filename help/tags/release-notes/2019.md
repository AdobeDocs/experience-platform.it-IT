...
L’API di [!DNL Reactor], che attiva i tag, oggi ha ufficialmente raggiunto lo stadio 1.0. Ci impegneremo a mantenere compatibilità con le versioni precedenti all’interno di questa versione principale (serie 1.x). Per iniziare a utilizzare [!DNL Reactor] API, visita il [Guida all’API di Reactor](../api/overview.md).
